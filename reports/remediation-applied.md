# Remediation APPLY plan — 2026-08-31T10:06:11.672Z
...
# Outcome (appended after execution, 2026-08-31)

## Database
- All 34 investigations patched (853 deterministic edits + quantum-ai brand evidence).
- Post-apply audit: **34/34 publish-gate failures → 2/34.** Score drift 32→0, metric
  literal drift 79→0, metric self-contradictions 2→0, unsupported definitive claims 21→0.
- The 2 remaining blocks are `STRONG_CLAIM_UNSOURCED` on `trade-vector-ai` and
  `whatsapp-bot` (zero sources on file while making external allegations) — attaching
  sources is analyst work; automating it would be fabrication.
- One detector fix fell out of the after-audit: "Do not deposit **without independent
  regulatory verification**" is a conditional (i.e. correct) directive and no longer flags.

## quantum-ai — now legitimately CONFIRMED
The review's own sources carried https://www.fca.org.uk/news/warnings/quantum-ai; fetched
live 2026-08-31 and verified: genuine FCA warning naming Quantum AI (published 2025-05-14,
updated 2026-03-03), listing the firm's domains. Recorded as `regulator_warnings` +
`regulators_checked`, with `primary_domain=quantumai.co` from the FCA page. Its
confirmed-scam language is therefore licensed and was deliberately NOT softened.

## Production (Replit) resync — 24 of 33 shipped
Via the deployed Vercel sync path (`resync-all-reviews.mjs`): **24 OK** (senvix,
quantum-ai, tradegpt, kaspi-ai, thistle-gainmere, peak-luxentria included; the last two
just run ~3-4 min each). **9 blocked by the deployed sync quality gate:**

| Review | Gate reason | Suggested path |
|---|---|---|
| affitto-casa-immobiliare, floventra, nordiqo, prestara-nexor, trade-vector-ai, whatsapp-bot | "no quality audit on record — run Polish before syncing" (pre-date the quality stack) | Run Polish from the admin UI, then sync |
| immediate-bienestar | audit VETO: hard-coded literals contradicting ground truth | Those literals were just fixed — re-run Polish; the veto should clear |
| primeaura, quarix-ai | audit VETO: unverified regulator claims/quotes | Genuine sourcing problem — analyst must source or remove the claims, then Polish |

Force-sync (`{force:true}`) exists but was NOT used — pushing YMYL pages past a quality
gate is an operator decision.

## Live-page spot check (senvix)
Meta description and headline now read **47/100, 18 countries, 107 impersonated figures**
(the corrected canon — 107 is the deduped named count). The threat-score BOX still shows
50/100 and one widget shows 17 — those elements render from Replit-side stored fields or
cache that the sync payload does not drive. Concrete, reproducible evidence for handoff
item 4.1: the Replit renderer must read the payload's canonical fields everywhere.

## Follow-ups queued
1. Polish + resync the 9 gate-blocked reviews (6 trivial, 1 should clear, 2 need sourcing).
2. Sources for trade-vector-ai + whatsapp-bot.
3. kaspi-ai: find the actual GOV.KZ warning URL naming Kaspi AI (its FAQ claim is softened
   until then; sources list only portal homepages).
4. review_translations rows still hold pre-remediation copies — the translation pipeline
   should re-run from the corrected masters.
5. Replit renderer work per `docs/REPLIT_PHASE1_INVESTIGATION_HANDOFF.md` (now with the
   senvix 47-vs-50 evidence).
6. Push the Phase 1 + remediation code via GitHub Desktop (your manual step) so the
   deployed pipeline gains the evidence-gated classification and validator.
