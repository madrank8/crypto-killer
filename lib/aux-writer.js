...
- avg scam score: {{platform_stat:avg_scam_score}}/100
- celebrity-impersonation brands: {{platform_stat:total_brands_with_celebrity_abuse}}
...
async function writeAux({
  ...
  // Primary: Sonnet (audit 2026-07-05, W4f: aux writes the ENTIRE SEO/schema
  // payload — internal links, citations, dataset, social proof, speakable —
  // running that on the weakest model in the map was a quality-first
  // mismatch. Was Haiku-primary.)
  const haikuStart = Date.now()
  try {
    const res = await callModel('claude-sonnet', system, user, { maxTokens, timeoutMs: timeouts.sonnetMs || timeouts.haikuMs })
    const parsed = extractJSON(res.text)
    if (validate(parsed)) {
      attempts.push({
        stage: 'aux', label: 'sonnet-primary', model: res.model, modelKey: 'claude-sonnet',
    ...
    attempts.push({
      stage: 'aux', label: 'sonnet-primary', model: 'claude-sonnet', modelKey: 'claude-sonnet',
      ok: false, durationMs: Date.now() - haikuStart,
      error: String(e?.message || e || 'unknown error').slice(0, 1500),
    })
  }

  // Retry: Opus (was Sonnet — pointless now that Sonnet is primary; the
  // retry should add model diversity, not re-roll the same dice)
  const sonnetStart = Date.now()
  try {
    const res = await callModel('claude-opus', system, user, { maxTokens, timeoutMs: timeouts.sonnetMs })
    const parsed = extractJSON(res.text)
    if (validate(parsed)) {
      attempts.push({
        stage: 'aux', label: 'opus-retry', model: res.model, modelKey: 'claude-opus',
    ...
    attempts.push({
      stage: 'aux', label: 'opus-retry', model: 'claude-opus', modelKey: 'claude-opus',
  ...
  return {
    ...
    // P2 fix (audit 2026-06-11): the previous fallback FABRICATED attributions
    // ("FTC Annual Report ${currentYear}") — a citation to a document nobody
    // checked exists. A deterministic fallback must never invent sourcing.
    // Empty social_proof is honest; the publish gate already warns on any
    // aux deterministic fallback so an editor reviews before publish.
    social_proof: [],
    visual_placeholders: [
