...
${contentSlugs.slice(0, 20).map((s) => `  - "${s.title}" → /blog/${s.slug}`).join('\n')}`
  ...
  const platformBlock = platformIntelligence?.totalBrands
    ? `CRYPTOKILLER PLATFORM DATA (use to populate dataset field if the article references first-party stats):
- ${platformIntelligence.totalBrands.toLocaleString()} scam brands tracked
- ${platformIntelligence.totalCreatives?.toLocaleString() || 'N/A'} ad creatives analyzed
- ${platformIntelligence.totalGeos || 'N/A'} countries
- avg scam score: ${platformIntelligence.avgScamScore || 'N/A'}/100`
    : ''
