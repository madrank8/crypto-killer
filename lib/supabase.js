...
/**
 * Pick the correct key based on the HTTP method and an explicit override.
 * Reads (GET/HEAD) default to the anon key (RLS-gated); writes use service_role
 * so RLS can restrict public writes while server-side mutations stay unblocked.
 * Pass `useServiceRole: true` to force service_role on a GET — required when
 * reading data RLS would otherwise hide (e.g., draft translations from admin
 * preview/edit paths). Falls back to anon if service_role is not configured.
 */
function getKeyForMethod(method, useServiceRole = false) {
 ...
 * Pass `useServiceRole: true` to bypass RLS on a GET (admin-only callers).
 */
async function supabaseRequest(path, options = {}) {
  ...
  // Strip our custom option before forwarding to fetch — fetch tolerates
  // unknown init props but emitting clean fetch init keeps the call surface tidy.
  const { useServiceRole, ...fetchOptions } = options
  const key = getKeyForMethod(fetchOptions.method, useServiceRole === true)
