  ...
  for (let i = 0; i < sections.length; i++) {
    ...
    if (visualMap[i]) {
      for (const vp of visualMap[i]) {
        ...
        if (match) {
          // Audit 2026-07-05 (A1): emit the RAW placeholder marker instead of
          // a styled "placeholder-box" figure. The fill pipeline now runs
          // lib/visual-generator processVisuals() over the BUILT html —
          // its PLACEHOLDER_REGEX matches this exact format and swaps in the
          // rendered chart/diagram/image (or a graceful fallback card on
          // failure). The old styled box was invisible to the generator (it
          // scanned section bodies, which the writer prompt forbids from
          // containing placeholders) and shipped literal grey boxes live.
          // If visual generation errors entirely, the publish gate blocks on
          // this marker instead of publishing it.
          sectionHtml += `\n<div class="visual-pending">[${match[1].toUpperCase()} NEEDED: ${match[2].trim()}${match[3] ? ` | Alt: ${match[3].trim()}` : ''}]</div>`
        }
      }
    }
