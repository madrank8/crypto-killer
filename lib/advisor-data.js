...
/* ─── Scraper / ad-intel summary (DB aggregates — no SpyOwl call) ─── */
async function scraperSection() {
  const brands = await supabaseRequest(
    '/scam_brands?select=slug,name,velocity_7d,velocity_trend,total_creatives,total_geos,scam_score,top_geo&order=velocity_7d.desc.nullslast&limit=20',
    { useServiceRole: true }
  )
  const list = brands || []
  const byTrend = {}
  for (const b of list) {
    const t = b.velocity_trend || 'unknown'
    byTrend[t] = (byTrend[t] || 0) + 1
  }
  return {
    source: 'scam_brands_db',
    top_by_velocity: list.slice(0, 12).map((b) => ({
      slug: b.slug,
      name: b.name,
      velocity_7d: b.velocity_7d,
      trend: b.velocity_trend,
    ...
    trend_counts_in_top20: byTrend,
  }
}

/* ─── Work-plan outcomes (lift vs baseline when available) ─── */
async function outcomesSection() {
  const rows = await supabaseRequest(
    '/work_plan_items?status=eq.done&select=fingerprint,action_type,target,title,executed_at,baseline_clicks,baseline_impressions,outcome_json&order=executed_at.desc.nullslast&limit=15',
    { useServiceRole: true }
  ).catch(() => [])
  return {
    recent_done: (rows || []).map((r) => ({
      fingerprint: r.fingerprint,
      action_type: r.action_type,
      target: r.target,
      title: r.title,
      executed_at: r.executed_at,
      baseline_clicks: r.baseline_clicks,
      baseline_impressions: r.baseline_impressions,
      outcome: r.outcome_json || null,
    })),
  }
}
...
export async function buildAdvisorSnapshot(days = 28) {
  const [traffic, search, content, opportunities, scraper, outcomes] = await Promise.all([
    ...
    scraperSection().catch((e) => ({ error: e.message })),
    outcomesSection().catch((e) => ({ error: e.message })),
  ])
