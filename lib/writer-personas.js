...
// Platform-stat token protocol — injected into every persona.systemPrompt
// BEFORE the "ARTICLE STRUCTURE:" anchor (see post-init mutation below) so
// every multi-agent writer (skeleton, section, faq, aux) and the monolithic
// generate path inherit the same guidance. Mirrors the {{stat:KEY}} review
// protocol from cryptokiller#17 / crypto-killer#18 — different namespace,
// different data, same pattern.
//
// The renderer (artifacts/crypto-review/src/lib/platformStatTokens.ts on
// the Replit side, cryptokiller#20) substitutes these tokens with current
// platform_aggregates values on every render. Writers emitting literal
// numbers freezes them into prose forever; emitting tokens keeps prose in
// sync with schema and dashboard indefinitely.
const STAT_TOKEN_PROTOCOL = `═══ PLATFORM-STAT TOKEN PROTOCOL ═══
The renderer substitutes \`{{platform_stat:KEY}}\` tokens with live aggregate values at render time. Use tokens (NOT literal numbers) when referencing CryptoKiller's cross-cutting platform stats — that way prose stays in sync with the schema and dashboard forever.

TOKENS TO USE (live, scraper-derived numbers — emit token, NOT the literal):
  {{platform_stat:total_brands_tracked}}              — scam brands tracked
  {{platform_stat:total_creatives_analyzed}}          — ad creatives analyzed
  {{platform_stat:total_brands_with_celebrity_abuse}} — brands using celebrity impersonation
  {{platform_stat:total_brands_reviewed}}             — published reviews on cryptokiller.org
  {{platform_stat:avg_scam_score}}                    — avg threat score (0-100)
  {{platform_stat:top_velocity_trend}}                — most common velocity ("surging", "stable", "declining")
  {{platform_stat:top_scam_brand_name}} / {{platform_stat:top_scam_brand_score}}

FORMAT MODIFIERS:  |short → "76k"   |raw → "76318"   default → "76,318"

KEEP LITERALS for: regulator counts, dates, dollar amounts cited from external sources, jurisdiction names, anything from a third-party source.

PLURALIZATION: tokens resolve to numbers whose plural form is unknowable at write time. Use plural-safe framings — "{{platform_stat:total_brands_tracked}} scam brands tracked", "the {{platform_stat:total_brands_tracked}}-brand portfolio", "across {{platform_stat:total_brands_tracked}} unique brands". Avoid "1 scam brand vs N scam brands" agreement traps.

INFORMATION GAIN: weave 2-3 of these tokens into body prose. Frame as: "CryptoKiller's analysis of {{platform_stat:total_brands_tracked}} scam brands shows…" or "Across {{platform_stat:total_creatives_analyzed}} ad creatives ingested…". This is your unique first-party data — no competing article has these numbers.

NEVER substitute snapshot values yourself; emit the LITERAL \`{{platform_stat:KEY}}\` token. Snapshot numbers freeze into the article forever; tokens stay live.`
...
// Topics whose readers are likely active or recent scam VICTIMS. Nair's
// mandated voice (pervasive sarcasm, deadpan, scare quotes) directly
// conflicts with the ICP tone rules ("Never mock. Validate suspicion.
// Address shame directly.") for these readers — a sardonic article in
// front of someone who just lost their savings is a brand-trust failure
// and a YMYL Needs-Met failure. Matched against title + target_keyword.
const VICTIM_FACING_RE = /\b(victim|recover(?:y|ing)?|get(?:ting)?\s+(?:your\s+)?money\s+back|lost\s+(?:money|savings|funds)|report(?:ing)?\s+(?:a\s+)?scam|chargeback|refund|what\s+to\s+do\s+(?:if|after|when)|after\s+(?:being|getting)\s+scammed|emotional|shame|grief|family\s+member)\b/i

/**
 * Select a persona, keyed to content intent.
 * Exclusion rules (in priority order):
 *   - Victim-facing topics (title/keyword match) exclude Nair — sarcasm
 *     never lands in front of victims (hard tone-safety rule)
 ...
 * @param {string} [opts.topicTitle] - topic title for victim-facing detection
 * @param {string} [opts.targetKeyword] - target keyword for victim-facing detection
 * @returns {Object} Selected persona object (webb, nair, or ortiz)
 */
function selectPersona(opts = {}) {
  const { contentType, pageRole, topicTitle = '', targetKeyword = '' } = opts
  let candidates = [webbPersona, nairPersona, ortizPersona]

  // HARD tone-safety rule: victim-facing topics never get the sardonic voice.
  if (VICTIM_FACING_RE.test(`${topicTitle} ${targetKeyword}`)) {
...
CryptoKiller has investigated {{platform_stat:total_brands_tracked}} scam brands across {{platform_stat:total_creatives_analyzed}} ad creatives.

Each line annotates the source: TOKEN values resolve live at render time
(emit the \`{{platform_stat:KEY}}\` literal in your prose); LITERAL values
are static-context only.

Emit each token EXACTLY as written. Values are deliberately withheld — a number you
write freezes into the article; a token resolves live on every sync.
- Total scam brands tracked: {{platform_stat:total_brands_tracked}}
- Total ad creatives analyzed: {{platform_stat:total_creatives_analyzed}}
- Average scam score across all brands: {{platform_stat:avg_scam_score}}/100
- Brands with celebrity impersonation: {{platform_stat:total_brands_with_celebrity_abuse}}
- Most common velocity trend: {{platform_stat:top_velocity_trend}}
PER-BRAND figures (this brand's own scraped data) have their own live tokens — use
them instead of writing a number, for the same reason: the scraper keeps ingesting
ads, so any literal you write goes stale within weeks.
- countries/geos targeted: {{stat:countries_targeted}}
- celebrities impersonated: {{stat:celebrities_abused}}
- ad creatives captured: {{stat:ad_creatives}}
- Published reviews on cryptokiller.org: {{platform_stat:total_brands_reviewed}}            [TOKEN — published count, computed live by Replit]
${pi.recentBrands ? `- Recently detected (last 7 days): ${pi.recentBrands}` : ''}
${pi.topScamScore ? `- Highest threat score brand: {{platform_stat:top_scam_brand_name}} ({{platform_stat:top_scam_brand_score}}/100)` : ''}
...
- TARGET KEYWORD must appear VERBATIM in the summary field AND within the first section body. Do not paraphrase or split the keyword phrase.
- H3 SUBHEADINGS: Sections longer than 200 words MUST include ### subheadings in the body text. Target 4-6 H3s total across the article. Use question format for H3s too where natural.
- ENTITY-SPECIFIC H3s: When discussing named people, platforms, or scam variants, give each notable entity its own H3 (e.g., "### The Martin Lewis Bitcoin Scam", "### How Binance Handles Fraud Reports"). This captures high-intent long-tail and regional searches.
...
// Inject the platform-stat token protocol into every persona's system prompt
// BEFORE the "ARTICLE STRUCTURE:" anchor. The 4 multi-agent writers
// (skeleton-writer, section-writer, faq-writer, aux-writer) all split
// persona.systemPrompt at /^ARTICLE STRUCTURE:/m and use only the prefix —
// inserting before that anchor ensures every writer in the pipeline (plus
// the monolithic generate path that uses the full prompt) inherits the
// protocol. Idempotent: a second module load replaces the placeholder
// rather than chaining; the regex is anchored on a literal that the
// protocol block does not contain.
for (const p of [webbPersona, nairPersona, ortizPersona]) {
  p.systemPrompt = p.systemPrompt.replace(
    /^ARTICLE STRUCTURE:/m,
    `${STAT_TOKEN_PROTOCOL}\n\nARTICLE STRUCTURE:`,
  )
}
