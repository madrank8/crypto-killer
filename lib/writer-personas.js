...
function selectPersona(opts = {}) {
  ...
  // Exclude Nair for long-form content (pillar pages, Root nodes, guides)
  const longFormTypes = ['pillar_page', 'guide', 'recovery_guide', 'comparison']
  if (pageRole === 'Root' || longFormTypes.includes(contentType)) {
...
Return JSON with this exact shape:
{
  ...
  "verify_tags_count": 0,
  "information_gain_summary": "1-2 sentences: what unique data does this page offer? Reference CryptoKiller platform data if provided.",
...
- 5-8 sections total
- 4-8 FAQ items
- internal_links must contain >= 2 items with real target_slug paths
