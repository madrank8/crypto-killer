...
async function cleanupStaleJobs() {
  ...
  const startedCutoff   = new Date(Date.now() - STARTED_STALE_MS).toISOString();
...
async function createJob(triggerType, geoFilter = null, startSkip = 0) {
  const now = nowIso();
  const jobData = {
    status: 'pending',
...
async function runScrapeLoop({
  ...
  while (hasMore && batchCount < maxBatches && !cancelled) {
    let page;
  ...
  const scrapeComplete = !hasMore && !cancelled;
  ...
  return {
    ...
    totalInserted,
    totalUpdated,
    hasMore: hasMore && !abortedEarly && !cancelled,
    nextSkip: skip,
    spyowlTotal,
    brandsUpdated,
    abortedEarly,
