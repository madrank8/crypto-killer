...
async function runScrapeLoop({
  ...
  while (hasMore && batchCount < maxBatches) {
    try {
      ...
      const pct = Math.min(10 + Math.round((globalFetched / (spyowlTotal || 1)) * 70), 80);
      await updateProgress(jobId, {
        phase: 'scanning',
        percent: pct,
        message: `Fetched ${globalFetched.toLocaleString()} of ${spyowlTotal.toLocaleString()} creatives...`,
        steps: [
  ...
  console.log(`[scraper] Scrape chunk done: ${totalFetched} creatives, aborted=${abortedEarly}, hasMore=${hasMore}`);
