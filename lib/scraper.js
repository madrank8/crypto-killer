...
import { HEARTBEAT_STALE_MS, buildStaleJobFilter } from '@/lib/stale-job-filter';

const SPYOWL_API = 'https://api.spyowl.icu';

// ─── Stale-job detection ───
// Reap only on a dead heartbeat (3 min). Do NOT reap because started_at is
// old: chained continue lambdas share one started_at, and a 6-minute cap
// killed healthy scrapes around skip 37k. Jobs with no heartbeat fall back
// to started_at using the same 3-minute window (see buildStaleJobFilter).
...
// the job is dead. Jobs with no heartbeat fall back to started_at using the
// same window. Crucially we PRESERVE progress (which contains next_skip) so
...
async function cleanupStaleJobs({ force = false } = {}) {
  ...
  // PostgREST `or=()`: stale heartbeat, or (no heartbeat AND old started_at).
  // ISO-8601 UTC timestamps sort lexicographically so `.lt.` is correct.
...
// ─── SpyOwl Creative Analytics (catalog volume summary) ───
// Live read of GET /creative/analytics — not part of the scrape loop.
// Auth failures throw with the same SpyOwl 401/403 prefix as fetchCreativePage
// so callers can short-circuit via isAuthFailureMessage.
async function fetchEnabledGeoRegions(cookie) {
  const url = `${SPYOWL_API}/geo-regions/enabled`;
  const res = await fetch(url, {
    headers: { Cookie: cookie },
    signal: AbortSignal.timeout(SPYOWL_TIMEOUT_MS),
  });
...
async function fetchCreativeAnalytics(cookie, { start, end, topLimit = 10, geoId = null } = {}) {
  ...
  const url = `${SPYOWL_API}/creative/analytics?${qs.toString()}`;
  const res = await fetch(url, {
    headers: { Cookie: cookie },
    signal: AbortSignal.timeout(SPYOWL_TIMEOUT_MS),
  });
