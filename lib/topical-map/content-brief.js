...
// Step 21 Entity Map (the Q-ID / sameAs half; schema_type ships in the row).
// Resolves the topic's primary entity against the curated wikidata-registry.
//
// HONESTY RULE (skill Step 21): "NEVER guess Q-IDs or sameAs URLs. Better
// unresolved than fabricated." So a registry MISS returns null — the brief simply
// carries no entity section rather than an entity with invented identifiers. An
// unresolved entity would add nothing anyway: its name is the title we already have.
//
// Matching is exact-slug (plus registry aliases) against a DOMAIN-CURATED registry,
// so a bare ambiguous token resolves to this domain's sense — 'gemini' is the crypto
// exchange, not Google's model. That is intended for a crypto-scam site. If the
// registry ever grows beyond this domain, matching needs a type/context qualifier
// before a bare single-token keyword can be trusted.
function resolvePrimaryEntity(topic) {
  ...
  for (const candidate of candidates) {
    ...
    return {
      ...
      type: entry?.type || schema?.['@type'] || 'Thing',
      // qid_override wins, exactly as buildSchemaEntity resolves sameAs. Some
      // registry rows carry a Wikidata *property* id in `qid` (e.g. bbb: P902)
      // with the real entity in qid_override — reading `qid` alone would publish
      // a property id as a "verified identifier" and contradict our own sameAs.
      wikidata_qid: entry?.qid_override || entry?.qid || null,
...
function buildContentBrief(topic, { parentTopic } = {}) {
  ...
  // ── Entity map (Step 21): only when the registry actually resolves it ───────
  const entity = resolvePrimaryEntity(t)
  if (entity) brief.entity = entity
...
function formatBriefForPrompt(topic, opts = {}) {
  ...
  if (b.entity) {
    const ids = [b.entity.wikidata_qid && `Wikidata ${b.entity.wikidata_qid}`, ...(b.entity.same_as || [])].filter(Boolean)
    lines.push(`- PRIMARY ENTITY: ${b.entity.name} (${b.entity.type})${ids.length ? ` — verified identifiers: ${ids.join(', ')}` : ''}. Treat this as the page's central entity; never invent other identifiers for it.`)
  }
