...
const { buildUrlPath, classifyContentFormat, classifySchemaType } = require('./publishing-metadata')
const { keywordMetricProvenance } = require('./provenance')

// Our own properties — never listed as "competitor pages to beat".
const OWN_DOMAINS = ['cryptokiller.org', 'crypto-killer.vercel.app']

// Match on the HOSTNAME, not a substring: `u.includes('cryptokiller.org')` would
// also exclude a genuine typosquat competitor like `mycryptokiller.org.evil.com`,
// which is exactly the kind of page this site exists to cover.
function isOwnUrl(u) {
  try {
    const host = new URL(u).hostname.toLowerCase().replace(/^www\./, '')
    return OWN_DOMAINS.some((d) => host === d || host.endsWith(`.${d}`))
  } catch {
    return false // unparseable URL is not ours; downstream filters handle it
  }
}
...
async function stageSerpClustering(run) {
  ...
  // Per-keyword SERP metadata for individual nodes later. urls/domains are the
  // measured top-10 organic results — they were previously fetched and thrown
  // away, which forced the content brief's Section 11 to render [NO DATA] for
  // competitor pages we had actually observed.
  const serpMeta = {}
  for (const [kw, s] of serpByKeyword.entries()) {
    serpMeta[kw] = { features: s.features, paa: s.paa, urls: s.urls || [], domains: s.domains || [] }
  }
...
async function stageLinking(run) {
  ...
  for (const f of flat) {
    ...
    n._paa = serp?.paa || []
    // Competitor pages actually observed ranking for this keyword. Our own domain
    // is not a competitor to beat, so it is filtered out.
    n._competitor_urls = (serp?.urls || [])
      .filter((u) => typeof u === 'string' && u && !isOwnUrl(u))
      .slice(0, 5)
...
async function stageSave(run, { supaFetch }) {
  ...
  const insertTopic = async (node, { parentId, topicType, section, nodeType, sortOrder, ancestorSlugs = [] }) => {
    let brandId = null
    let reviewId = null
    ...
    const leafSlug = uniqueSlug(node._slug || node.slug_hint || node.title)
    const row = {
      map_id: mapId,
      parent_id: parentId,
      ...
      node_type: nodeType,
      node_function: classifyNodeFunction({ content_type: node.content_type, content_role: node.content_role, search_intent: node._metrics?.search_intent, node_type: nodeType, topic_type: topicType }),
      // Tier 2 publishing metadata (deterministic; skill Steps 15-19 + v4.1 Content Format)
      url_path: buildUrlPath([...ancestorSlugs, leafSlug]),
      content_format: classifyContentFormat({ format_code: node.format_code, search_intent: node._metrics?.search_intent, node_type: nodeType, content_type: node.content_type }),
      schema_type: classifySchemaType({ content_type: node.content_type, format_code: node.format_code }),
      ...
      competitor_urls: node._competitor_urls || [],
    ...
    return { id: item?.id || null, slug: leafSlug }
  }
    ...
    for (const c of branch.clusters || []) {
      ...
      for (const s of c.supporting || []) {
        ...
        await insertTopic(s, {
          ...
          nodeType: 'standard',
          sortOrder: si++,
          ancestorSlugs: [pillar.slug, cluster.slug],
        })
        topicCount += 1
      }
    }
