...
// A caller-supplied cadence object must carry a POSITIVE rate: a rate of 0 or
// less would never advance the chunking loop below (infinite loop), so it falls
// back to the default rather than hanging.
function resolveCadence(cadence) {
  if (cadence && typeof cadence === 'object' && Number.isFinite(cadence.perWeek) && cadence.perWeek > 0) return cadence
...
function buildPublicationPlan(topics, options = {}) {
  ...
  // Floor of 1: the loop below advances by `rate`, so a non-positive rate must
  // never reach it regardless of how the cadence was supplied.
  const requested = Number.isFinite(perWeek) && perWeek > 0 ? Math.floor(perWeek) : resolved.perWeek
  const rate = Number.isFinite(requested) && requested > 0 ? Math.floor(requested) : CADENCES[DEFAULT_CADENCE].perWeek
