...
function isSupportingPageRow(page) {
  if (page?.topic_type === 'supporting') return true
  ...
  // Unclassified sheet page rows become supporting after consolidate
  return true
}

function requiresInternalLinks(page) {
  return isSupportingPageRow(page)
}
...
function validateImportedPages(pages) {
  ...
  ;(pages || []).forEach((page, idx) => {
    ...
    const missing = []
    if (!sheetCell(page, 'Page Title (Title Tag Style)')) {
      missing.push('Page Title (Title Tag Style)')
    }
    if (!sheetCell(page, 'Suggested URL')) missing.push('Suggested URL')
    if (!sheetCell(page, 'Section')) missing.push('Section')
    if (!sheetCell(page, 'Cluster')) missing.push('Cluster')
    ...
    if (isSupportingPageRow(page) && !sheetCell(page, 'Internal Links To')) {
