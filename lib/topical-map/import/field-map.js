...
function keywordFromTitle(title) {
  const before = String(title || '').split(':')[0]
...
function sheetMetricProvenance(source, { hasVolume, hasKd }) {
  const base = keywordMetricProvenance(source)
  return {
    ...base,
    search_volume: hasVolume ? base.search_volume : 'unresolved',
    keyword_difficulty: hasKd ? base.keyword_difficulty : 'unresolved',
  }
}
...
function mapPageRow(rawRow) {
  ...
  const kws = splitKeywords(row['Primary Query Cluster'])
  ...
  const hasKd = kd !== null
  const keywordDataSource = hasVolume || hasKd ? 'spreadsheet' : 'unverified'

  return {
    title,
    url_path: urlPath,
    slug: leafSlugFromUrl(urlPath, title),
    target_keyword: kws[0] || keywordFromTitle(title),
    secondary_keywords: kws.slice(1),
    search_volume: volume ?? 0,
    keyword_difficulty: hasKd ? kd : null,
    ...
    metric_provenance: sheetMetricProvenance(keywordDataSource, { hasVolume, hasKd }),
...
function buildTopicFields(node, { topicType, section, parentUrlPath, ancestorSlugs = [], sortOrder = 0, clusterLabel = null }) {
  ...
  const kdMissing = node.keyword_difficulty == null
  const priority = computeTopicPriorityScore({
    search_volume: node.search_volume ?? 0,
    keyword_difficulty: kdMissing ? 50 : node.keyword_difficulty,
  ...
  if (node.url_path) {
    urlPath = node.url_path
  } else if (node.url_path === null || topicType === 'cluster') {
    urlPath = null
  } else if (parentUrlPath) {
    urlPath = null
  } else {
    urlPath = `/${leafSlug}/`
  }
  ...
  return {
    ...
    search_volume: node.search_volume ?? 0,
    keyword_difficulty: kdMissing ? null : node.keyword_difficulty,
    business_value: businessValue,
