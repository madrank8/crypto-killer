...
//      never erases it. Gather-stack may now fill field_observation_count /
//      recurring_pattern when a cited SpyOwl query or quote exists; otherwise
//      those fields stay missing for a human.
//
...
async function loadSupportingTopics(supaFetch, mapId) {
  const rows = await supaFetch(
    `/topics?map_id=eq.${mapId}&topic_type=eq.supporting&select=id,title,slug,url_path,content_type,content_format,notes,target_keyword,internal_links_to,content_status,content_id,review_id&order=sort_order.asc`
  )
...
async function upsertContentBrief({ supaFetch, mapId, topicId, contentType, forcingInputs, sullivanOk, hasExisting }) {
  // Evidence fields only. Never set `status` on PATCH: approved / in-production /
  // published briefs must not be demoted by a readiness re-run.
  const row = {
    topic_id: topicId,
    map_id: mapId,
  ...
  if (hasExisting) {
    ...
    await supaFetch('/content_briefs', {
      method: 'POST',
      headers: { Prefer: 'return=minimal' },
      body: JSON.stringify({ ...row, status: 'draft' }),
    })
  }
}
 ...
 */
async function processTopic({ topic, mapId, supaFetch, fetchImpl, firecrawlApiKey, classifyType }) {
  // Already linked to a published article/review: no brief churn.
  if (
    topic.content_status === 'published' ||
    topic.content_id ||
    topic.review_id
  ) {
    return {
  ...
  const contentType = existingBrief?.content_type || classifyType || proposeSullivanType(topic)
