...
function fromKeywordDataSource(source) {
  // keyword_data_source can be a single provider ('dataforseo', 'ahrefs') OR a
  // compound like 'dataforseo+ahrefs' when a DataForSEO gap is backfilled with
  // Ahrefs (stages.js). Any tool provider means the metric was measured.
  // 'spreadsheet' = human-curated sheet import (estimated, not tool-measured).
  const s = String(source || '')
  if (s.includes('dataforseo') || s.includes('ahrefs')) return PROVENANCE.MEASURED
  if (s.includes('llm') || s.includes('spreadsheet')) return PROVENANCE.ESTIMATED
  return PROVENANCE.UNRESOLVED
}
function keywordMetricProvenance(source) {
  const level = fromKeywordDataSource(source)
  return { search_volume: level, keyword_difficulty: level, cpc: level, volume_trend_yearly: level, traffic_potential: level }
}
