...
function filterClustersByCannibalization(clusters, ctx) {
  ...
  const survivorKeys = new Set(survivors.map((c) => c.cluster_key))
  const prunedSurvivors = pruned.filter((p) => survivorKeys.has(p.cluster_key))

  return { survivors, dropped, pruned: prunedSurvivors, report }
}
