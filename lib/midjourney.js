...
async function pollForResult(taskId, options = {}) {
  const { onProgress, maxWaitMs = MAX_POLL_MS, intervalMs = POLL_INTERVAL_MS, abortSignal } = options

  const start = Date.now()
  let attempt = 0

  while (Date.now() - start < maxWaitMs) {
    // Allow external abort (e.g. when function timeout is approaching)
    if (abortSignal?.aborted) {
      throw new Error(`MJ poll aborted after ${Math.round((Date.now() - start) / 1000)}s (function timeout approaching)`)
    }
...
async function generateMidjourneyImage(rawPrompt, options = {}) {
  const { aspectRatio = '16:9', style = '', mode = 'fast', onProgress, maxWaitMs, maxRetries = MAX_RETRIES } = options
  ...
  for (let retry = 0; retry <= maxRetries; retry++) {
    try {
      ...
      // 2. Poll for result
      const result = await pollForResult(job.task_id, {
        onProgress,
        ...(maxWaitMs ? { maxWaitMs } : {}),
      })
