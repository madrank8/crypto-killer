...
const SECTIONS = Object.freeze([
  ...
  },
  {
    ...
    fields: ['h1', 'h1_coverage_manifest', 'bluf_target', 'heading_seed_provenance', 'heading_structure'],
  },
...
// Anything that could be misread by a YAML parser gets quoted. The `[` case is the
// one that matters most here — see the header note.
...
function emitEntry(key, value, depth, lines) {
  ...
  if (Array.isArray(value)) {
    if (value.length === 0) { lines.push(`${indent}${key}: []`); return }
...
function toYaml(brief) {
  ...
  const lines = [
    '# ─────────────────────────────────────────────────────────────',
    '# CONTENT BRIEF',
    '# Generated by: crypto-killer admin (content-brief-generator v1.4 port)',
