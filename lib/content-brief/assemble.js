...
// (content-brief-generator v1.6, references/brief-template.md).
//
...
//
// v1.6 additions: locale + orthography_notes (Section 3), ple_unit on every
// heading row, faq_sweep (Section 6 remainder), escalation-ladder CTA guidance
// for YMYL closing H2s.
...
// Verified against the INSTALLED skills (~/.claude/skills/…). Update when the
// downstream pipeline is upgraded — a brief that claims the wrong target version
// is a checkable fact rendered without checking.
const SEO_BLOG_GENERATOR_VERSION = '5.4'
const TOPICAL_MAP_VERSION = '4.9'
const CONTENT_BRIEF_GENERATOR_VERSION = '1.6'

// Crypto Killer ships English-US by default. Non-default markets must set locale
// explicitly (skill v1.6) — never invent a market orthography rule.
const DEFAULT_LOCALE = 'en-US'

function pendingPleUnit() {
  return {
    pixel: PLACEHOLDER.PENDING_LLM,
    letter: PLACEHOLDER.PENDING_LLM,
    byte: PLACEHOLDER.PENDING_LLM,
  }
}
...
function assembleBrief(input = {}) {
  ...
  // v1.6 — BCP-47 market tag. Prefer an explicit topic/map locale; otherwise site default.
  brief.locale = isStr(topic.locale) ? topic.locale.trim() : DEFAULT_LOCALE
  brief.orthography_notes = isStr(topic.orthography_notes)
    ? topic.orthography_notes.trim()
    : ''
  ...
  // NOTE: this is Sullivan SC-098 content_type — NOT topics.content_type (pillar_page, …).
  ...
  // Every heading row must carry all Option-B per-heading fields (template: "never
  // omit the field") plus v1.6 ple_unit. `_seed_source` is internal provenance,
  // stripped at YAML export.
  const headingBlock = (h2, seedSource, over = {}) => ({
    ...
    ple_unit: pendingPleUnit(),
    _seed_source: seedSource,
    ...over,
  })
  const ctaInstruction = ymyl
    ? 'YMYL escalation ladder (v1.6): (1) self-serve / self-check step → (2) when Crypto Killer / investigation help fits → (3) honest limit — when it does not (in-person counsel, bank/regulator) → (4) emergency / irreversible-loss signs + locale-native contact path. Never invent emergency numbers. End with the CTA link.'
    : PLACEHOLDER.PENDING_LLM
  ...
  // v1.6 — query-network remainder → FAQ H3s. Empty until enrich (or SERP capture)
  // supplies real unmapped clusters. Never invent volume.
  brief.faq_sweep = {
    carrier_h2: `Common Questions About ${primaryKeyword || topic.title || 'This Topic'}`,
    items: [],
  }
...
module.exports = {
  PLACEHOLDER,
  WORD_COUNT_BY_FORMAT,
  DEFAULT_LOCALE,
  CONTENT_BRIEF_GENERATOR_VERSION,
  pendingPleUnit,
  assembleBrief,
}
