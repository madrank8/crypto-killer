...
// Target version pinned to the INSTALLED skill (~/.claude/skills/seo-blog-generator/SKILL.md
// → "Current version: v5.4"). Not the illustrative "3.1" in the brief template's
// example block. Writer/auditor comments are pinned to v5.4; gates 6.7b / 6.9c,
// YMYL TIER ledger, and locale scan SC-105/106 are NOT ported.
...
function assembleBrief(input = {}) {
  ...
  // v1.6 locale contract. This site ships en-US; do not invent a market tag.
  brief.locale = ''
  brief.orthography_notes = ''
  ...
  brief.heading_structure = [
    ...
    headingBlock('[Action-Oriented Final H2 — not "Conclusion"]', 'mandatory_template', {
      _mandatory: true,
      inline_link: PLACEHOLDER.PENDING_LLM,
      source_ledger_seeds: [],
      instruction:
        'YMYL escalation ladder: (1) self-check / what the reader can verify alone → (2) when Crypto Killer investigation content fits → (3) when it does not (named professional or regulator, not this site) → (4) FBI IC3 https://www.ic3.gov/ and FTC ReportFraud https://reportfraud.ftc.gov/. Never invent an emergency phone number. US-English default; locale is empty.',
    }),
  ]
  brief.heading_seed_provenance = paa.length ? 'serp_paa (measured)' : PLACEHOLDER.DERIVED_NOT_SERP
  brief.faq_sweep = {
    carrier_h2: `Common questions about ${isStr(topic.title) ? topic.title : 'this topic'}`,
    items: [],
  }
  const toolish = /check|tool|bot|calculator/i.test(`${topic.title || ''} ${topic.content_format || ''} ${topic.url_path || ''}`)
  brief.anti_mistake = toolish
    ? {
        items: [
          {
            question: `Should you use this tool for ${primaryKeyword || 'this topic'}?`,
            answer_target: PLACEHOLDER.PENDING_LLM,
          },
          {
