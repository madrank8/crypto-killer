...
const { PLACEHOLDER, pendingPleUnit } = require('./assemble')
...
const ENRICHABLE = Object.freeze([
  ...
  'faq_sweep',          // v1.6 — unmapped query remainder; volumes never invented
])

// Fields the model may NEVER write: measured metrics, deterministic map data,
// human-supplied Sullivan evidence, locale contract, and identity/lifecycle.
...
function guardPleUnit(value, report) {
  if (!value || typeof value !== 'object' || Array.isArray(value)) {
    report.push({ field: 'ple_unit', reason: 'invalid ple_unit shape — kept pending scaffold' })
    return pendingPleUnit()
  }
  const out = pendingPleUnit()
  for (const k of ['pixel', 'letter', 'byte']) {
    if (value[k] === undefined) continue
    out[k] = scrubDeep(value[k], report, `ple_unit.${k}`)
  }
  return out
}
...
function guardHeadingStructure(assembled, incoming, report) {
  ...
  const merged = existing.map((row) => {
    ...
    for (const f of HEADING_WRITABLE) {
      ...
      if (f === 'source_ledger_seeds') {
        ...
        next[f] = guardPleUnit(match[f], report)
      } else {
        next[f] = match[f]
      }
    }
    ...
    if (!next.ple_unit) next.ple_unit = pendingPleUnit()
    return next
  })
  ...
  for (const [h2, row] of byH2) {
    ...
    for (const f of HEADING_WRITABLE) {
      if (f === 'source_ledger_seeds') {
        added[f] = guardClaimList(row[f], report)
      } else if (f === 'ple_unit') {
        added[f] = row[f] !== undefined ? guardPleUnit(row[f], report) : pendingPleUnit()
      } else {
        added[f] = row[f] ?? PLACEHOLDER.PENDING_LLM
      }
    }
...
/**
 * faq_sweep items: keep question/answer text (scrubbed); never invent volumes.
 * Numeric volumes from the model are replaced with NO_DATA unless already NO_DATA.
 */
function guardFaqSweep(assembled, incoming, report) {
  const base = assembled && typeof assembled === 'object' ? assembled : { carrier_h2: '', items: [] }
  if (!incoming || typeof incoming !== 'object' || Array.isArray(incoming)) {
    report.push({ field: 'faq_sweep', reason: 'invalid faq_sweep shape — kept assembled scaffold' })
    return base
  }
  const carrier = isStr(incoming.carrier_h2)
    ? scrubDeep(incoming.carrier_h2, report, 'faq_sweep.carrier_h2')
    : (base.carrier_h2 || '')
  const rawItems = Array.isArray(incoming.items) ? incoming.items : []
  const items = []
  for (const item of rawItems) {
    if (!item || typeof item !== 'object') continue
    if (!isStr(item.question) && !isStr(item.query_cluster)) continue
    let volume = PLACEHOLDER.NO_DATA
    if (item.volume === PLACEHOLDER.NO_DATA || item.volume === PLACEHOLDER.UNVERIFIED) {
      volume = item.volume
    } else if (typeof item.volume === 'number' || (isStr(item.volume) && /^\d+$/.test(item.volume.trim()))) {
      // Model-supplied volumes are unverified in this pipeline (no SERP capture yet).
      report.push({ field: 'faq_sweep.items.volume', reason: 'invented volume discarded', original: item.volume })
      volume = PLACEHOLDER.NO_DATA
    } else if (isStr(item.volume)) {
      volume = scrubDeep(item.volume, report, 'faq_sweep.items.volume')
    }
    items.push({
      question: scrubDeep(item.question || item.query_cluster, report, 'faq_sweep.items.question'),
      query_cluster: isStr(item.query_cluster)
        ? scrubDeep(item.query_cluster, report, 'faq_sweep.items.query_cluster')
        : scrubDeep(item.question, report, 'faq_sweep.items.query_cluster'),
      volume,
      answer_target: isStr(item.answer_target)
        ? scrubDeep(item.answer_target, report, 'faq_sweep.items.answer_target')
        : PLACEHOLDER.PENDING_LLM,
      schema_eligible: item.schema_eligible === false ? false : true,
    })
  }
  return { carrier_h2: carrier, items }
}
...
function mergeEnrichment(assembled, llmOutput) {
  ...
  for (const key of Object.keys(src)) {
    ...
    } else if (key === 'faq_sweep') {
      next = guardFaqSweep(base[key], value, rejected)
...
6. The two headings marked mandatory must keep their role: one names who this is NOT for (include a line a competitor would never publish), the final one is action-oriented, never titled "Conclusion". For YMYL pages structure the final CTA as an escalation ladder (self-serve → when help fits → honest limit → emergency/irreversible-loss signs). Never invent emergency numbers.
7. Never invent search-volume numbers for faq_sweep.items.volume — use exactly: ${PLACEHOLDER.NO_DATA}
8. Sullivan content_type is SC-098 (${brief.content_type || 'unset'}) — it is NOT the map page format (${brief.content_format || topic.content_type || 'unset'}). Do not confuse them.
9. Polarity matrix (v1.6): for scam-type / symptom / risk entities, enumerate with/without poles as H3s or faq_sweep items when they are real long-tail questions — never invent volume.
10. Anti-mistake nodes: for remedy/tool topics seed boolean "Should you use X for Y?" items in faq_sweep or as H3s, plus one "what to avoid" item when relevant.
11. Each heading must include ple_unit: { pixel, letter, byte } describing visual container, prose register, and structured-data layer as one topical unit. No fabricated CDN image URLs in pixel.
...
Output schema (emit only these keys):
{
  ...
  "visual_assets": [{"type":"diagram|comparison_table|chart|screenshot|infographic|lookup_table","description":"","alt_text":"","placement":"after [H2 title]"}],
...
Content format (map / page format — NOT Sullivan): ${brief.content_format}
...
NON-COMMODITY ANGLE — Sullivan SC-098 (human-supplied — build the brief around this, do not contradict it):
sullivan_content_type: ${brief.content_type || '(none)'}
