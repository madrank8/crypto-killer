...
// Choose the source type for itemReviewed.@type.
//
// The schema's itemReviewed must describe the SAME entity the editorial prose
// does. The editorial `item_reviewed.type` (the writer's evidence-grounded
// judgement of what the scam actually is — a fake trading platform is a
// 'FinancialProduct') is what readers see; when the stored
// `brandData.entity_type` disagrees (e.g. a curated 'SoftwareApplication' for a
// fake trading platform), the schema must follow the editorial, not contradict
// it — that class-level divergence is exactly what the quality auditor flags.
//
// Prefer the editorial type when it is a recognised schema.org type; fall back
// to the stored brand classification, then to resolveItemReviewedType's safe
// 'Organization' default. The returned value is always Google-Rich-Results
// valid (FinancialProduct/InvestmentFund → Product, Service → Organization).
function pickItemReviewedType(reviewContent, brandData) {
  const editorial = reviewContent?.item_reviewed?.type
  const clean = typeof editorial === 'string' ? editorial.trim() : ''
  if (clean && ENTITY_TYPE_TO_GOOGLE[clean]) return resolveItemReviewedType(clean)
  return resolveItemReviewedType(brandData?.entity_type)
}
...
function orgSameAs() {
  // Delegates to lib/entity-registry.js — one definition of who we are.
  return require('./entity-registry').organizationSameAs()
}
 ...
 *
 * itemReviewed's @type follows the editorial `reviewContent.item_reviewed.type`
 * (mapped to a Google-valid type), falling back to `brandData.entity_type`, then
 * to 'Organization' — see pickItemReviewedType. This keeps the schema and the
 * visible prose describing the same entity class.
 */
function buildReviewSchema({
  ...
  if (!threat) {
    try {
      const { classifyThreat, brandEvidence } = require('./threat-score')
      threat = classifyThreat(brandData?.scam_score ?? 0, brandEvidence(brandData), {
        override: brandData?.classification_override || null,
      })
    } catch {
  ...
  const itemReviewedType = pickItemReviewedType(reviewContent, brandData)
  ...
  // ── Author identity (Phase 1 review fix) ──────────────────────────────
  // The Person node follows the VISIBLE byline. When the byline names one of
  // the five real analysts (matched by name or persona id via the entity
  // registry), the schema author becomes that analyst's canonical Person —
  // anchored on their live /author/<id> page — so the byline, the Person node
  // and the author page finally describe the same human. When the byline is
  // the collective "Crypto Killer Research Team" (all 34 current rows), the
  // long-standing team node below is kept verbatim, so existing pages do not
  // change until someone actually assigns an analyst.
  const entityRegistry = require('./entity-registry')
  const bylineAnalyst = entityRegistry.resolveAnalyst(
    reviewContent.author_name || reviewContent.author_persona_id
  )
  const authorId = bylineAnalyst.path
    ? `${entityRegistry.siteUrl()}${bylineAnalyst.path}#person`
    : `${siteUrl}/#author`
  ...
  return {
    ...
    '@graph': [
      ...
      // Defined once in lib/entity-registry.js and reused by the schema
      // builder, the rendered footer/byline and the analyst pages, so the
      // visible page and the structured data cannot describe two different
      // publishers.
      require('./entity-registry').organizationEntity(),
      ...
      // topic. Falls back to the analyst's registry expertise, then to a
      // generic crypto-fraud list on the team node.
      bylineAnalyst.path
        ? entityRegistry.analystEntity(bylineAnalyst.id, {
            description: reviewContent.expertise_depth || undefined,
            knowsAbout:
      ...
      // ── Article Entity (primary content) ──
      {
        ...
        // Article.headline is required for the rich result and must never be
        // empty. Falling back to the canonical page title keeps the structured
        // data matching the visible H1 shape rather than emitting ''. (Phase 1)
        headline:
          reviewContent.headline ||
          reviewContent.title ||
          `${brandData.name} Review: Is ${brandData.name} a Scam?`,
        ...
        about: [
          {
            // Same entity as Review.itemReviewed — keep the @type identical so
            // WebPage.about and the Review node never describe different classes.
      ...
      // ── Review Entity ──
      // itemReviewed @type follows the editorial item_reviewed.type (see
      // pickItemReviewedType), falling back to brandData.entity_type — so the
      // schema and the visible prose always describe the same entity class. No
      // more hardcoded "Cryptocurrency investment platform" description leaking
