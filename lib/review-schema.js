...
//
// NEXT_PUBLIC_SITE_URL is honored as an override (useful for dev / preview
// environments) but defaults to the canonical URL. Matches the pattern in
// lib/sync-shape.js:1068.
function getSiteUrl() {
  return process.env.NEXT_PUBLIC_SITE_URL || 'https://cryptokiller.org'
}

// Valid schema.org types for itemReviewed on a scam review. Anything
// outside this set falls back to 'Thing'. We intentionally don't accept
// 'Product' or 'Service' without an entity_type hint from the brand row
// — "Product" on a rental scam is the Affitto Casa bug.
  ...
  return {
    ...
    '@graph': [
      ...
      // ── WebPage Entity (was MISSING — required by schema-markup-generator) ──
      {
        ...
        inLanguage: 'en-US',
        ...(dataset ? { mainEntity: { '@id': `${reviewUrl}#spyowl-dataset` } } : {}),
        speakable: {
      ...
      // ── Article Entity (primary content) ──
      {
        ...
        // SpyOwl dataset node (emitted below). Making this edge explicit
        ...
        // the Article node.
        ...(dataset
          ? { isBasedOn: { '@id': `${reviewUrl}#spyowl-dataset` } }
          : {}),
      },
      ...
      // ── Dataset Entity (SpyOwl intelligence) ──
      //
