...
function buildReviewSchema({
  ...
  return {
    ...
    '@graph': [
      ...
      ...(reviewContent.how_to
        ? [
            {
              '@type': 'HowTo',
              '@id': `${reviewUrl}#howto-protect`,
              name: reviewContent.how_to.name,
              description: reviewContent.how_to.description,
              ...(reviewContent.how_to.totalTime ? { totalTime: reviewContent.how_to.totalTime } : {}),
              step: reviewContent.how_to.steps.map((s, i) => ({
                '@type': 'HowToStep',
                position: i + 1,
                name: s.name,
                text: s.text,
