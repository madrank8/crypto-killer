...
// ─── Blocked source/citation URLs ───────────────────────────────────────
// Mirrors sync-shape.js STATIC_BLOCKED_CITATION_DOMAINS. The REVIEW path
// strips these via normalizeCitations, but the content/article path shipped
// sources & citations to Replit unfiltered — so a Google grounding-API
// redirect (vertexaisearch.cloud.google.com/grounding-api-redirect/...) that
// leaked from writer output would render as an opaque "source" on the live
// blog. This is the Floventra-class leak the review path was hardened
// against; the article path now gets the same guard.
const STATIC_BLOCKED_SYNC_DOMAINS = [
  'cryptopygious.cloud.google.com',
  'grounding-api-redirect',
  'vertexaisearch.cloud.google.com',
]

function isBlockedSyncUrl(url) {
  if (typeof url !== 'string') return true
  const lower = url.toLowerCase()
  return STATIC_BLOCKED_SYNC_DOMAINS.some((d) => lower.includes(d))
}

// Drop any entry whose url is blocked/malformed, preserving the object shape
// otherwise (the Replit content renderer relies on the existing field names).
function stripBlockedUrlEntries(raw) {
  if (!Array.isArray(raw)) return raw
  return raw.filter((entry) => {
    if (!entry || typeof entry !== 'object') return false
    if (typeof entry.url !== 'string' || !entry.url.startsWith('http')) return false
    return !isBlockedSyncUrl(entry.url)
  })
}
...
function shapeContentForSync(content) {
  ...
  // Strip grounding-redirect / known-exfiltration URLs from the source ledger
  // and citations before they reach the live blog (parity with the review
  // path). Only touched when present, so legacy rows are unaffected.
  if (Array.isArray(content.sources)) shaped.sources = stripBlockedUrlEntries(content.sources)
  if (Array.isArray(content.citations)) shaped.citations = stripBlockedUrlEntries(content.citations)
