  ...
  try {
    ...
    if (researchCandidates.length) {
      ...
      try {
        ...
        sendStep(deps, { step: 'surgical', status: 'failed', message })
        unfixable.push({
          key: 'surgical_model_error',
          reason: message,
          operator_action:
            'Surgical model failed; continuing with deterministic/soften. Retry Fix & Publish after model access is fixed.',
        })
      }
    }
    ...
    // Soften needs the auditor's narrative hard_fail_reason (quoted spans / figures),
    // not only the generic evaluateHardFails reason strings.
    const auditBlob =
      (workingRow?.ai_audit &&
        workingRow.ai_audit.hard_fail_checks &&
        workingRow.ai_audit.hard_fail_checks.hard_fail_reason) ||
      workingRow?.ai_audit?.hard_fail_reason ||
      (Array.isArray(ctx.gateReasons) ? ctx.gateReasons.join(' ') : '') ||
      ''
    const failsForSoften = remainingFails.map((f) => ({
      ...f,
      reason: [f.reason, auditBlob].filter(Boolean).join(' | '),
    }))
    ...
    if (remainingFails.length > 0 && softenableRemaining.length > 0) {
      ...
      const softened = softenFn(workingRow, failsForSoften) || {}
