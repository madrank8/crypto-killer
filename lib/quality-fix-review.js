...
function resolveReviewHardFails(row) {
  ...
  if (checks && typeof checks === 'object') {
    ...
    return evaluateHardFails(audit, row).failed
  }

  if (row.audit_hard_fail !== true) return []
  ...
  if (found.length === 0) {
    found.push({ key: 'any_hard_fail', reason })
  }
  return found
}
...
function buildReviewAgentDeps({
  ...
  async function runSurgicalModel(prompt) {
    ...
    const result = await callModel('gpt-5.4', system, user, {
      jsonMode: true,
      effort: 'high',
      timeoutMs: 120000,
