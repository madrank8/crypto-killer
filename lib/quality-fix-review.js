...
const { stripFailedVisualPlaceholders, unwrapVisualPending } = require('./visual-placeholders')
...
function scrubVisualPlaceholders(text) {
  ...
  return unwrapVisualPending(stripFailedVisualPlaceholders(text))
...
function remediateReviewDeterministic(row, hardFails, context = {}) {
  ...
  for (const fail of fails) {
    ...
    if (/visual placeholder|placeholder-box|ck-visual--placeholder|\[\s*(chart|diagram|image|infographic|screenshot|photo|step-by-step)\s+needed/i.test(reasonBlob)) {
    ...
    if (
      ...
      !/visual placeholder|placeholder-box|ck-visual--placeholder|\[\s*(chart|diagram|image)/i.test(reasonBlob)
    ) {
