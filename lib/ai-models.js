...
// Claude models that default to effort: 'high' adaptive thinking. Without an
// explicit override, 6–8k-token article generation can take 3+ minutes and
// blow past lambda timeouts. We default these to 'low'; callers can pass
// options.effort to bump it back up for tasks that benefit from deeper
// reasoning (e.g. quality audit, complex synthesis).
// Reference: https://docs.claude.com/en/docs/about-claude/models/migrating-to-claude-4
...
async function callAnthropic(model, systemPrompt, userPrompt, options = {}) {
  ...
  // so long-form writers don't spend the entire request thinking. Callers can
  // explicitly pass options.effort = 'medium' | 'high' to bump it.
  if (HIGH_EFFORT_DEFAULT_MODELS.has(model.model)) {
    body.effort = options.effort || 'low'
 ...
 *   effort: 'low' | 'medium' | 'high' — overrides the default thinking effort
 *           on Claude 4.6+ models (defaults to 'low' there for predictable
 *           latency). No effect on OpenAI/Google providers.
