...
//   watch the changelog)
// - GPT-5.4 flagship pinned for quality-fix agent (high-effort audits)
...
async function callOpenAI(model, systemPrompt, userPrompt, options = {}) {
  const res = await fetchWithTimeout('https://api.openai.com/v1/chat/completions', {
    ...
    body: JSON.stringify({
      model: model.model,
      // Audit 2026-07-05 (W4a): GPT-5.x chat completions require
      // `max_completion_tokens`; `max_tokens` 400s on those models — which
      // is why the gpt-5.4-mini auditor pin was silently abandoned. Send
      // the modern param for gpt-5* models, legacy for anything older.
      ...(String(model.model).startsWith('gpt-5')
        ? { max_completion_tokens: options.maxTokens || model.maxTokens }
        : { max_tokens: options.maxTokens || model.maxTokens }),
      // GPT-5 reasoning models accept `reasoning_effort` (minimal|low|medium|
      // high). Callers pass options.effort; forward it for gpt-5* so a
      // "high-effort" audit actually reasons harder. 'max' clamps to 'high'
      // (OpenAI's ceiling). Non-gpt-5 models ignore effort as before.
      ...(String(model.model).startsWith('gpt-5') && options.effort
        ? { reasoning_effort: options.effort === 'max' ? 'high' : options.effort }
        : {}),
      messages: [
