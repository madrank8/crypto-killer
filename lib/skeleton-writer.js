...
function buildSkeletonSystem(persona, currentYear, discoverMode = false) {
...
═══ RULES ═══
${discoverMode
  ? `- GOOGLE DISCOVER MODE (W5a): the SUMMARY sets up the story WITHOUT resolving the core question — tease the payoff, don't deliver it. The body's final section resolves it. (Inverse of the usual declaration-first rule; intentional for feed engagement.)`
...
═══ FIELD CONSTRAINTS ═══
${discoverMode
  ? `- title: HOOK title ≤ 70 chars — curiosity gap that makes a feed-scroller stop. Never fabricate; the intrigue must be real and paid off in the article.
- headline: H1 mirroring the hook energy, keyword natural if it fits, ≤ 90 chars`
...
async function writeSkeleton({
  ...
  const discoverMode = topic?.content_type === 'discover'
  const system = buildSkeletonSystem(persona, currentYear, discoverMode)
