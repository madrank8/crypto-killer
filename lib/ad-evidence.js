...
/**
 * Resolve STRUCTURED ad-creative evidence for a brand (capped). Returns
 *   { images: [{ geo, celebrity, url }], geoCounts: { GEO: count } }
 * for the renderer to display as a dedicated client + SSR section — replacing
 * the full_article HTML injection (which only rendered in SSR, never the React
 * client). Reuses the same fetch/cache pipeline. Returns null when there's no
 * evidence. Never throws.
 */
export async function resolveAdEvidence({ brand, limit = MAX_EVIDENCE_IMAGES }) {
  try {
    if (!brand?.normalized_name) return null
    const { images, geoCounts } = await resolveEvidenceImages(brand, limit)
    if (!images.length) return null
    return {
      images: images.map((i) => ({ geo: i.geo, celebrity: i.celebrity, url: i.url })),
      geoCounts,
    }
  } catch {
    return null
