...
const SANITIZE_PATTERNS = [
  ...
  [/ransomware/gi, 'security threat'],
  [/ponzi/gi, 'investment scheme'],
 ...
 * On content-policy rejection, retries with a generic safe prompt.
 */
async function generateImage(description, altText) {
  ...
  try {
    ...
    if (!res.ok) {
      ...
      // Content-policy rejection → retry with ultra-safe generic prompt
      if (res.status === 400) {
        console.log('[Visual] DALL-E 400 — retrying safe prompt...')
