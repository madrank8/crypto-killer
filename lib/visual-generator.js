 ...
 * On content-policy rejection (400) or rate-limit (429), retries with safe prompt.
 */
async function generateImage(description, altText) {
  ...
  try {
    ...
    console.error(`[Visual] DALL-E FAILED: ${err.message}`)    // Last-ditch: try safe retry even on unexpected errors (network, timeout)
    try {
...
async function uploadToSupabaseStorage(imageUrl, storagePath) {
  ...
  try {
    ...
    console.log(`[Visual] Uploaded successfully: ${publicUrl}`)    return publicUrl
  } catch (err) {
...
async function createStorageBucket() {
  try {
    ...
    if (res.ok) {
      ...
      const errText = await res.text()      console.error(`[Visual] Bucket creation response: ${errText.slice(0, 200)}`)
    }
  } catch (err) {
...
function buildVisualHtml(visual) {
  ...
  // Diagrams use mermaid.ink PNG endpoint — render with dark bg padding like charts
  ...
  <img src="${visual.url}" alt="${escapeHtml(visual.altText)}" loading="lazy" width="${visual.width || 700}" style="max-width:100%;height:auto;border-radius:12px;border:1px solid rgba(148,163,184,0.2);${isDiagram ? 'background:rgb(15,23,42);padding:1rem;' : ''}" />
