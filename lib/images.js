 ...
 */
async function generateImage({ type = 'hero', seed = '', customQuery = '', filename = '', onProgress, maxMjWaitMs } = {}) {
 ...
 */
async function generateImageSet(slug, { contentCount = 1, onProgress, maxMjWaitMs } = {}) {
  const errors = [];
 ...
 */
async function generateArticleImages(slug, article, { contentCount = 2, aiHelpers, onProgress, maxMjWaitMs } = {}) {
  const errors = []
  ...
  const heroPromise = generateImage({
    ...
    try {
      return await generateImage({ type: 'hero', seed: slug, filename: `hero-${slug}`, maxMjWaitMs })
    } catch (retryErr) {
