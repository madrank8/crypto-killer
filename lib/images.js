/**
 * Image Pipeline: Imagen 4.0 → Midjourney → Unsplash → TinyPNG → Supabase Storage
 *
 * Generates high-quality editorial photography via Google Imagen 4.0 (primary),
 * falls back to Midjourney via APIFrame, then Unsplash as last resort.
 * All images are compressed via TinyPNG and uploaded to Supabase Storage.
 *
 * Fallback chain: Imagen 4.0 Fast (~5-8s) → Midjourney (~60-150s) → Unsplash (~3s)
 */
...
async function searchUnsplash(query, { count = 5, orientation = 'landscape' } = {}) {
  ...
  return (data.results || []).map(photo => ({
    ...
    raw: photo.urls?.raw, // full-res, Imgix-resizable (for >=1200px hero crops)
...
// Build a properly-sized Unsplash (Imgix) URL. Google Discover large cards need
// the hero >=1200px wide; `urls.regular` is only 1080px at the photo's native
// ratio, so heroes are forced to 1600x900 (16:9) and content to 1600x1067 (3:2),
// cropped toward faces/entropy. Falls back to the regular URL if raw is absent.
function unsplashSized(photo, type = 'hero') {
  const raw = photo.raw || photo.url;
  if (!raw) return photo.url;
  const sep = raw.includes('?') ? '&' : '?';
  const box = type === 'hero' ? 'w=1600&h=900' : 'w=1600&h=1067';
  return `${raw}${sep}${box}&fit=crop&crop=faces,entropy&q=80&fm=jpg`;
}

/**
 * Generate an image via Imagen 4.0 (primary) → Midjourney → Unsplash (last resort).
 *
 * Full pipeline: Generate → Compress → Upload → Return URL.
 *
 * Imagen 4.0 Fast: ~5-8s, excellent photorealistic quality, article-specific.
 * Midjourney: 60-150s (usually skipped on Vercel due to 60s timeout).
 * Unsplash: ~3s, but limited to existing stock photos (often irrelevant).
 *
 * @param {Object} options
 * @param {'hero'|'content'} options.type - Image purpose
 * @param {string} [options.seed] - Seed string for consistent prompt selection
 * @param {string} [options.customQuery] - Override the prompt/query (used for Imagen/MJ prompts)
 ...
 */
async function generateImage({ type = 'hero', seed = '', customQuery = '', filename = '', onProgress, maxMjWaitMs, maxMjRetries, imagenOnly = false } = {}) {

  // ═══════════════════════════════════════════════════════
  // PRIMARY: Google Imagen (~5-15s depending on model tier)
  // ═══════════════════════════════════════════════════════
  if (GOOGLE_AI_API_KEY) {
    ...
    try {
      console.log(`[images] Generating ${type} via Imagen: "${prompt.slice(0, 80)}..."`)

      const imagenResult = await generateImagenImage(prompt, {
        aspectRatio: type === 'hero' ? '16:9' : '4:3',
      });

      // Compress the Imagen output (PNG, often 1-3MB)
      let compressed;
      try {
        compressed = await compressWithTinyPNG(imagenResult.buffer);
      } catch (compErr) {
        console.warn('[images] Compression failed, using raw Imagen image:', compErr.message);
        compressed = { buffer: imagenResult.buffer, size: imagenResult.buffer.length, originalSize: imagenResult.buffer.length, ratio: 1, type: imagenResult.mimeType };
      }

      // Embed AI-provenance metadata (IPTC trainedAlgorithmicMedia) into the
      // bytes before upload. Must run after compression — TinyPNG strips
      // metadata. (Audit: AI-disclosure pipeline.)
      const ext = compressed.type?.includes('png') ? 'png' : 'jpg';
      const taggedBuffer = tagAiGeneratedImage(compressed.buffer, {
      ...
      const fname = filename || `${type}-imagen-${Date.now()}`;
      ...
      return {
        ...
        // Keep visible captions human-friendly; AI tooling disclosure belongs in schema metadata.
        credit: 'CryptoKiller editorial illustration',
        creditUrl: null,
        query: prompt,
      ...
      console.error(`[images] Imagen ${type} failed: ${imagenErr.message}`);
      if (imagenOnly) {
        // Caller opted out of fallbacks entirely — propagate the error so they
        // can decide whether to skip this image or surface the failure.
        throw new Error(`Imagen failed: ${imagenErr.message}`);
      }
      console.error('[images] — trying next source');
    }
  } else if (imagenOnly) {
    throw new Error('imagenOnly set but GOOGLE_AI_API_KEY is not configured');
  }

  // ═══════════════════════════════════════════════════════
  // FALLBACK 1: Midjourney via APIFrame (slow, usually skipped)
  // ═══════════════════════════════════════════════════════
  if (APIFRAME_API_KEY) {
    const prompts = type === 'hero' ? HERO_PROMPTS : CONTENT_PROMPTS;
    const prompt = customQuery || pickQuery(prompts, seed);
    ...
    try {
      ...
      // Embed AI-provenance metadata before upload (after compression).
      const ext = compressed.type?.includes('png') ? 'png' : 'jpg';
      const taggedBuffer = tagAiGeneratedImage(compressed.buffer, {
        mimeType: compressed.type,
        creatorTool: 'Midjourney',
        credit: 'AI-generated illustration (Midjourney)',
      });
      ...
      console.error(`[images] Midjourney ${type} failed: ${mjErr.message} — trying Unsplash`);
    }
  }

  // ═══════════════════════════════════════════════════════
  // FALLBACK 2: Unsplash (last resort)
  // ═══════════════════════════════════════════════════════
  ...
  console.log(`[images] Unsplash fallback for ${type}: "${query}"`)

  let photos;
  try {
    photos = await searchUnsplash(query, { count: 5, orientation: 'landscape' });
  } catch (searchErr) {
  ...
  const compressed = await compressWithTinyPNG(unsplashSized(photo, type));
  const ext = compressed.type?.includes('png') ? 'png' : 'jpg';
...
async function generateImageQueries(article, aiHelpers, { forceUnsplash = false } = {}) {
  ...
  // Imagen and MJ both handle rich descriptive prompts; only use Unsplash queries as last resort
  const useRichPrompts = !!(GOOGLE_AI_API_KEY || (APIFRAME_API_KEY && !forceUnsplash));

  const system = useRichPrompts
    ? `You generate image prompts that look like REAL editorial photography — NOT AI art.
These prompts will be used with Google Imagen or Midjourney to generate photorealistic images.
Return ONLY valid JSON — no markdown fences.
{
  "hero_query": "A detailed editorial photography prompt for the hero image",
  "hero_alt": "Descriptive alt text for the hero image, specific to the article topic (10-15 words)",
  "section_queries": ["prompt for section 1", "prompt for section 2", ...],
  "section_alts": ["alt text for section 1 image", "alt text for section 2 image", ...]
}
Rules:
...
- Each prompt should be SPECIFIC to the article topic — reference the actual subject matter.
...
Generate ${useRichPrompts ? 'detailed editorial photography prompts' : 'Unsplash search queries'} — each one must be specific to the article content above, not generic.`
  ...
  try {
    ...
    if (parsed?.hero_query && Array.isArray(parsed?.section_queries)) {
      return {
        ...
        heroAlt: parsed.hero_alt || '',
        sectionQueries: parsed.section_queries,
        sectionAlts: parsed.section_alts || [],
        queryType: useRichPrompts ? 'imagen' : 'unsplash',
      }
    }
  } catch (err) {
 ...
 */
async function generateArticleImages(slug, article, { contentCount = 2, aiHelpers, onProgress, maxMjWaitMs, maxMjRetries, imagenOnly = false } = {}) {
  const errors = []

  if (imagenOnly && !GOOGLE_AI_API_KEY) {
    throw new Error('imagenOnly set but GOOGLE_AI_API_KEY is not configured')
  }

  // Step 1: Generate context-aware prompts
  // When imagenOnly is set we always want rich editorial prompts from the AI,
  // not the short Unsplash-style queries.
  const skipMJ = maxMjWaitMs != null && maxMjWaitMs <= 1
  const forceUnsplash = !imagenOnly && skipMJ && !GOOGLE_AI_API_KEY
  let queries = { heroQuery: '', sectionQueries: [], queryType: 'unsplash' }
  try {
    queries = await generateImageQueries(article, aiHelpers, { forceUnsplash })
  ...
  // Override generic Unsplash alt text with AI-generated article-specific alt text
  if (hero && queries.heroAlt) {
    hero.alt = queries.heroAlt
  }
  if (queries.sectionAlts?.length > 0) {
    contentImages.forEach((img, i) => {
      if (queries.sectionAlts[i]) {
        img.alt = queries.sectionAlts[i]
      }
    })
  }
