...
async function generateImage({ type = 'hero', seed = '', customQuery = '', filename = '', onProgress, maxMjWaitMs, maxMjRetries } = {}) {
  ...
  // ═══════════════════════════════════════════════════════
  // PRIMARY: Google Imagen 4.0 Fast (~5-8s)
  // ═══════════════════════════════════════════════════════
  if (GOOGLE_AI_API_KEY) {
    ...
    try {
      ...
      console.error(`[images] Imagen ${type} failed: ${imagenErr.message} — trying next source`);
      // Fall through to Midjourney/Unsplash
    }
  }
...
async function generateArticleImages(slug, article, { contentCount = 2, aiHelpers, onProgress, maxMjWaitMs, maxMjRetries } = {}) {
  ...
  // Only force Unsplash queries if BOTH Imagen AND MJ are unavailable
  const skipMJ = maxMjWaitMs != null && maxMjWaitMs <= 1
  const forceUnsplash = skipMJ && !GOOGLE_AI_API_KEY
