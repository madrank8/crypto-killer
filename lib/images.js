...
async function generateImage({ type = 'hero', seed = '', customQuery = '', filename = '', onProgress, maxMjWaitMs, maxMjRetries } = {}) {
  ...
  // Don't use customQuery for Unsplash — it may be a long MJ-style prompt
  // that returns zero results on Unsplash. Use short fallback queries instead.
  const query = pickQuery(queries, seed);
