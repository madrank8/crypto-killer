...
function enforceNumericConsistency(reviewContent, canonical, mode = 'autofix') {
  ...
  // how_to.description + how_to.steps[].text
  if (reviewContent.how_to && typeof reviewContent.how_to === 'object') {
    ...
    if (Array.isArray(reviewContent.how_to.steps)) {
      reviewContent.how_to.steps = reviewContent.how_to.steps.map((s, i) => {
        if (!s || typeof s !== 'object') return s
        return {
          ...s,
          text: typeof s.text === 'string' ? fixInString(s.text, `how_to.steps[${i}].text`) : s.text,
        }
      })
    }
  }
