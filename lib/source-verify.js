...
const HEAD_403_OK = new Set([
  ...
  // The FCA's main site (Wave-2 regen, 2026-08-31) serves HTTP 403 to every
  // automated HEAD request — including its own warning pages that a browser
  // GET returns fine (verified live: /news/warnings/quantum-ai,
  // /consumers/warning-list-unauthorised-firms). The FCA is the single most
  // cited regulator in these investigations; without this entry the gate
  // rejects every genuine FCA warning citation. register.fca.org.uk answers
  // HEAD normally and stays fully verified.
  'fca.org.uk', 'www.fca.org.uk',
])

// Audit 2026-07-05 (A8): 5s was too aggressive for bot-hostile gov hosts
// (sec.gov, ic3.gov routinely exceed it) — transient timeouts were then
// misread as dead sources by the publish gate.
...
async function headCheckUrl(url) {
  ...
  // Audit 2026-07-05 (R13): Google AI grounding redirects were blocked in
  // normalizeCitations (sync-shape) but sailed through THIS ledger check —
  // they HEAD-200 and then ship into the visible "Sources & References"
  // sidebar as opaque redirect URLs. Same class as Floventra's citation leak,
  // one field over. Drop them at the ledger gate.
  if (host.includes('vertexaisearch') || url.includes('grounding-api-redirect')) {
    return { ok: false, reason: `grounding_redirect: '${host}' is a Google AI search-grounding redirect, not a citable source — resolve to the destination URL or drop` }
  }
  try {
    ...
    // Audit 2026-07-05 (A8): distinguish TRANSIENT network failures
    // (timeouts, resets — common on bot-hostile gov hosts) from PERMANENT
    // ones (DNS not found, connection refused). Publish gates hard-block on
    // 'network:'; transient failures get the 'network-transient:' prefix so
    // they soft-warn instead.
    const msg = String(e.message || e.name || 'unknown error')
    const isTransient = /timeout|timed out|aborted|ECONNRESET|ETIMEDOUT|socket hang up/i.test(msg) || e.name === 'TimeoutError'
    return { ok: false, reason: `${isTransient ? 'network-transient' : 'network'}: ${msg}` }
  }
}
  ...
  try {
    ...
    if (total > 0) {
      return {
        ...
        type: 'regulatory',
        verified: true,
        // Deliberately QUALITATIVE, no raw count. A full-text hit only means the
        // phrase appears in some filing — for a generic brand name ("Quantum AI")
        // those are almost always unrelated legitimate filings, NOT SEC scrutiny
        // of this scam, and the count drifts on every re-lookup. Presenting it as
        // a number invites the writer to copy a misleading literal. The exact
        // count is kept in lookup.hits for internal use only.
        relevance: `SEC EDGAR full-text search returns matches for the phrase "${brandName}", but a full-text mention is not evidence of SEC registration, endorsement, or enforcement — the matches may be unrelated filings.`,
        extract: `The phrase "${brandName}" appears in SEC EDGAR full-text search results; this is not evidence the entity is registered with or scrutinised by the SEC.`,
...
async function fcaWarningListProbe(brandName) {
  ...
  for (const slug of candidates) {
    ...
    try {
      // Audit 2026-07-05 (R12): this used to be a HEAD check — a bare 200
      // (slug collision with a DIFFERENT similarly-named firm, or a CDN
      // serving soft-404s as 200) injected a false "the FCA has warned about
      // this brand" claim into the ledger, which the writer then states as
      // fact. Maximum defamation exposure. Now: GET the page and require the
      // actual brand name in the body before asserting anything.
      const res = await fetch(url, {
        method: 'GET',
        signal: AbortSignal.timeout(CHECK_TIMEOUT_MS),
        redirect: 'follow',
      ...
      if (res.ok) {
        const bodyText = (await res.text()).slice(0, 200000)
        // Name match: case-insensitive, whitespace-tolerant. Require the
        // full brand name (not just a token) to appear in the page.
        const nameRx = new RegExp(
          brandName.trim().replace(/[.*+?^${}()|[\]\\]/g, '\\$&').replace(/\s+/g, '\\s+'),
          'i'
        )
        if (!nameRx.test(bodyText)) {
          console.warn(`[source-verify] FCA warning page ${url} returned 200 but does NOT name "${brandName}" — slug collision or soft-404, skipping`)
          continue
        }
        return {
