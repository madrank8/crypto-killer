...
function contentWriterPrompt(brandData, creativeSample, longevityDays, currentDate, sourceLedger, availableImages, cleanCelebrityList, passedThreat) {
...

{
  ...
  "dataset": "REQUIRED for frameAsScam=true. For other tiers, provide best-available or null. Shape: { name, description, keywords[], creator, measurementTechnique, variableMeasured[], temporalCoverage }. Use: { \"name\": \"SpyOwl ${brandData.name} Scam Intelligence Dataset\", \"description\": \"${derivedCelebCount} celebrities impersonated across ${brandData.total_creatives || 'N'} creatives in ${brandData.total_geos || 'M'} countries over ${longevityDays} days\", \"keywords\": [\"cryptocurrency fraud\", \"celebrity impersonation\", \"scam intelligence\"], \"creator\": {\"@type\": \"Organization\", \"name\": \"CryptoKiller\"}, \"measurementTechnique\": \"Automated ad-creative surveillance across Meta, Google, TikTok, X ad libraries\", \"variableMeasured\": [\"Ad Creative Count\", \"Celebrity Impersonations\", \"Country Targets\", \"Campaign Duration (days)\", \"Weekly Creative Velocity\"], \"temporalCoverage\": \"${brandData.first_seen_at}/${brandData.last_seen_at}\" }. The Dataset node is CryptoKiller's highest-leverage information-gain signal.",

  "item_list": "REQUIRED. Shape: { name, description, numberOfItems, itemListOrder: 'Descending', items: [{position, name, description}] }. Items mirror the red_flags array as a ranked ItemList. Provide 3-6 items matching red_flags cardinality.",
  ...
  "claims": "REQUIRED. Array of ClaimReview-shaped claims. CORE claim for any scam review: {\"claimReviewed\": \"${brandData.name} is a legitimate investment platform.\", \"appearance\": [\"ad-campaigns\", \"fake-social-proof\"], \"reviewRating\": {\"ratingValue\": 1, \"bestRating\": 5, \"worstRating\": 1, \"alternateName\": \"False\"}, \"author\": \"CryptoKiller Research Team\"}. Add 1-2 more specific evidence-backed claims. Do NOT invent claims — only what the evidence supports.",
...
- Celebrities Impersonated (CANONICAL — use this EXACT number everywhere): ${derivedCelebCount}
...
═══ PLURALIZATION & NUMBER AGREEMENT ═══
Match noun number to count: "1 country" / "2 countries". "1 day" / "6 days". "1 celebrity" /
"2 celebrities". "1 creative" / "28 creatives". NEVER "1 countries" / "1 days". When count
is 1, use the singular noun. Applies to all fields (headlines, stats, stage footers, FAQ,
dataset.description, verdicts).
