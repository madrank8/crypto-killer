...
// ── non-evidentiary-source guard (source_ledger_claims_without_links) ────────
//
...
// fraud claim on a YMYL page — drop it from the evidence list. (The writer may
// still mention it in protection prose.) ScamAdviser is the recurring offender.
//
// Deliberately NARROW: it does NOT sweep "any source not inline-linked". That
// rule is unsafe here — inline source links are added by the deterministic HTML
// scaffold AFTER this remediation runs, so at remediation time the writer's
// prose can legitimately lack them, and a broad sweep would wrongly delete every
// source (observed: a weak writer roll with zero inline links → sources emptied).
// An uncited *legitimate* source is left for the audit veto; only the
// non-evidentiary set is ever removed. Never invents a source.
...
/**
 * Drop known non-evidentiary aggregator sources from the evidence list.
...
function filterUncitedSources(review) {
  ...
  const kept = review.sources.filter((s) => {
    ...
    if (dom && NON_EVIDENTIARY_SOURCE_DOMAINS.has(dom)) {
      dropped.push((s && s.title) || (s && s.url) || 'unknown')
      return false
    }
    return true
  })
