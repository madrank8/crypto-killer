...
// ── uncited-source guard (source_ledger_claims_without_links hard-fail) ──────
//
// A source present in sources[] but not tied to a body claim trips the publish
// gate. Two deterministic, honest resolutions:
//   • an uncited source (its domain is not inline-linked anywhere in the prose)
//     is not evidence we used — drop it from the evidence list;
//   • a known non-evidentiary aggregator (a crowd trust-score site) is a
//     report-to/checker resource, not an authoritative citation for a specific
//     fraud claim on a YMYL page — drop it regardless of an incidental homepage
//     link. (The writer may still mention it in protection prose.)
// Never invents a source; only removes ones that cannot legitimately stay.
const NON_EVIDENTIARY_SOURCE_DOMAINS = new Set(['scamadviser.com'])

function registrableDomain(url) {
  try { return new URL(String(url)).hostname.replace(/^www\./, '').toLowerCase() } catch { return '' }
}

// Collect every href="…" URL from all string leaves of a value.
function collectHrefs(value, out, depth = 0) {
  ...
  if (Array.isArray(value)) { for (const v of value) collectHrefs(v, out, depth + 1); return }
  if (typeof value === 'object') { for (const k of Object.keys(value)) collectHrefs(value[k], out, depth + 1) }
}

/**
 * Drop sources that cannot legitimately remain in the evidence list.
 * @returns { sources, dropped: [title] }
 */
function filterUncitedSources(review) {
  if (!review || !Array.isArray(review.sources)) return { sources: review && review.sources, dropped: [] }
  // Cited domains = domains inline-linked anywhere in the prose (NOT the
  // sources array itself, which lists the URLs without being a citation).
  const hrefs = new Set()
  for (const k of Object.keys(review)) if (k !== 'sources') collectHrefs(review[k], hrefs)
  const citedDomains = new Set([...hrefs].map(registrableDomain).filter(Boolean))

  const dropped = []
  const kept = review.sources.filter((s) => {
    const dom = registrableDomain(s && s.url)
    const reason = !dom ? 'no-url'
      : NON_EVIDENTIARY_SOURCE_DOMAINS.has(dom) ? 'non-evidentiary'
        : !citedDomains.has(dom) ? 'uncited'
          : null
    if (reason) { dropped.push((s && s.title) || (s && s.url) || 'unknown'); return false }
    return true
  })
...
function remediateReview(review, context = {}) {
  ...
  // (roster-anchored; body prose stays for the audit veto). Runs before the
  // source filter so hrefs in a dropped item can't keep a source alive.
  ...
  // Drop uncited / non-evidentiary sources so an unlinked seeded source can't
  // trip the source_ledger_claims_without_links publish gate.
  let sourcesDropped = []
  if (r.sources !== undefined) {
    const { sources, dropped } = filterUncitedSources(r)
    r.sources = sources
