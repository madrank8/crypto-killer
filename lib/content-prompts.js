 ...
 * Canon: ai-brain seo-blog-generator. Comments pinned to **v5.4** (Aug 2026).
 * Kill lists + AI disclosure are in this repo; the following v5.4 gates are
 * NOT ported: Step 6.7b brief-fidelity-gate, Step 6.9c helpful-content-gate,
 * YMYL TIER ledger, locale scan SC-105/106. Do NOT claim full skill parity.
 */
...
Emit each token below EXACTLY as written. The values are deliberately NOT shown to
you: a number you write becomes frozen into the article forever, while a token is
resolved to the live value on every sync. You do not need to know the magnitude to
write the sentence.
...
${pi.recentBrands ? `- Recently detected (last 7 days): ${pi.recentBrands}   [LITERAL — no token exists; use only as background, do NOT print this number]` : ''}
...
EMIT THE TOKEN LITERAL. Never write a platform-wide figure as a number — if you find
yourself typing a digit for a CryptoKiller aggregate, you need a token instead.
