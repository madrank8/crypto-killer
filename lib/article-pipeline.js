  ...
  const sectionResults = await Promise.all(
    sections.map(async (section, sectionIndex) => {
      ...
      const result = await writeSection({
        ...
        // W5d: sources + platform stats reach body prose (all sections —
        // the writer only cites what fits the section's scope).
        sourceLedger,
        platformStats: platformIntelligence,
        // W5a: Discover lane — delayed answer, dopamine close on the last
        // section.
        discoverMode: topic?.content_type === 'discover',
        isFinalSection: sectionIndex === sections.length - 1,
      })
