...
import { classifyThreat, brandEvidence, dedupeCelebrityList } from '@/lib/threat-score'
import { normalizeUpdateHistory } from '@/lib/update-history'
 ...
 * @returns {object|null}
 */
function normalizeItemReviewed(raw, { brandName, brandOrigin } = {}) {
  ...
  const description = typeof raw.description === 'string' && raw.description.trim()
    ...
    : null
  const url = sanitizeItemReviewedUrl(
...
export function shapeReviewForSync(review, brand, options = {}) {
  ...
  // Evidence rides along so an 80+ brand WITH a regulator warning on file can
  // actually reach CONFIRMED language at sync time — the score alone never can.
  const threat = classifyThreat(resolvedScore, brandEvidence(brand), {
  ...
  const brandOrigin = pickBrandOrigin([
    ...(Array.isArray(optionsLandingUrls) ? optionsLandingUrls : []),
    ...(Array.isArray(brand?.landing_urls) ? brand.landing_urls : []),
    ...(Array.isArray(options.recentAds)
      ? options.recentAds.map((a) => a?.link_url).filter(Boolean)
      : []),
  ...
  const hasStructuredTakeaways = Array.isArray(review.key_takeaways) && review.key_takeaways.length > 0
  const hasStructuredFaq = Array.isArray(review.faq) && review.faq.length > 0

  let normalizedFullArticle = normalizeFullArticleForIntegrity(
    normalizeFullArticleForTier(
      typeof review.full_article === 'string' ? review.full_article : '',
      threat,
    ),
  )
  // Structured key_takeaways / faq are the sole render source on Replit.
  // Strip matching blocks from full_article so speakable hits one DOM node.
  if (hasStructuredTakeaways || hasStructuredFaq) {
    normalizedFullArticle = stripStructuredDupesFromArticle(normalizedFullArticle, {
      hasKeyTakeaways: hasStructuredTakeaways,
      hasFaq: hasStructuredFaq,
    })
  }
  const fullArticleLength = normalizedFullArticle.length
  const fullArticleHash = sha256Hex(normalizedFullArticle)
  const fullArticleWordCount = countPlainWords(normalizedFullArticle)
  ...
  // SEO-safe title/meta: Replit appends " | CryptoKiller" to <title>, so budget 45.
  // Replit MUST use seo_title/title for document <title>, never hard-slice headline.
  const seo = composeSeoFields({ title: titleText, meta: metaText })
  titleText = seo.title
  metaText = seo.meta_description
  const seoTitle = seo.seo_title

  // Word count / reading time from post-strip article (single source of truth).
  const syncWordCount = fullArticleWordCount || (review.word_count ?? 0)
  const syncReadingMinutes = syncWordCount
    ? Math.max(1, Math.round(syncWordCount / 200))
    : 0
  ...
  const shaped = {
    ...
    // ── Visible update provenance (2026-07-08) ──
    // The article's "Update history" block: append-only entries written by
    // generate (regenerated), polish (visuals_updated), the admin editor
    // (edited), publish (published) and the nightly stats-refresh cron
    // (stats_update, material deltas only). Replit renders newest-first
    // near the byline and MUST derive Article.dateModified from the newest
    // entry's date — not from row timestamps — so a visible change is the
    // only thing that ever moves the date (anti fake-freshness).
    update_history: normalizeUpdateHistory(review.update_history),
    stats_refreshed_at: review.stats_synced_at || null,
    ...
    word_count: syncWordCount,
    reading_minutes: syncReadingMinutes,
    ...
    //   - link_domain   (hostname only — full link_url NEVER shipped)
    //   - post_url      (Facebook post permalink when available)
    //   - fp_link       (Facebook page URL)
    //   - cta_url/label/rel — ONLY safe public CTA (FB post or Ad Library; never landers)
    // CTA safety (Peak Luxentria audit 2026-08): never ship raw link_url.
    // Prefer cta_url = Facebook post_url, else Meta Ad Library search, else null.
    // Replit MUST use cta_url for the card CTA — never link_url / landing_urls.
    recent_ads_sample: Array.isArray(options.recentAds)
      ? options.recentAds.slice(0, 20).map((a) => {
          ...
          // Defense in depth: never leave an unsafe landing URL in any string field.
  ...
  // ── Resolve {{platform_stat:KEY}} tokens at sync time (2026-07-08) ──
  // The live Legacy Bitfundex page rendered a RAW
  // "{{platform_stat:total_brands_tracked}}" in the Methodology section:
  // Replit's render-time resolver doesn't cover every key the writer
  // personas are told to emit. Per-brand {{stat:*}} tokens stay untouched
  // (Replit resolves those reliably); platform-wide tokens get substituted
  // here with the aggregates the caller fetched, so a raw token can never
  // reach the public page. Values refresh on every re-sync. When the
  // caller couldn't fetch aggregates, tokens pass through unchanged —
  // no worse than before.
  if (options.platformStats) {
    const ps = options.platformStats
    const tokenValues = {
      total_brands_tracked: Number.isFinite(ps.totalBrandsTracked) ? ps.totalBrandsTracked.toLocaleString('en-US') : null,
      total_creatives_analyzed: Number.isFinite(ps.totalCreativesAnalyzed) ? ps.totalCreativesAnalyzed.toLocaleString('en-US') : null,
      total_brands_with_celebrity_abuse: Number.isFinite(ps.totalBrandsWithCelebrityAbuse) ? ps.totalBrandsWithCelebrityAbuse.toLocaleString('en-US') : null,
      avg_scam_score: Number.isFinite(ps.avgScamScore) ? String(ps.avgScamScore) : null,
      top_velocity_trend: typeof ps.topVelocityTrend === 'string' ? ps.topVelocityTrend : null,
      top_scam_brand_name: typeof ps.topScamBrandName === 'string' ? ps.topScamBrandName : null,
      top_scam_brand_score: Number.isFinite(ps.topScamBrandScore) ? String(ps.topScamBrandScore) : null,
    }
    const resolvePlatformTokens = (value) => {
      if (typeof value === 'string') {
        return value.replace(/\{\{platform_stat:([a-z0-9_]+)\}\}/gi, (match, key) => {
          const v = tokenValues[key.toLowerCase()]
          return v != null ? v : match
        })
      }
      if (Array.isArray(value)) return value.map(resolvePlatformTokens)
      if (value && typeof value === 'object') {
        const out = {}
        for (const [k, v] of Object.entries(value)) out[k] = resolvePlatformTokens(v)
        return out
      }
      return value
    }
    const resolved = resolvePlatformTokens(shaped)
    // Token substitution changes full_article's byte length, so the
    // integrity fields computed pre-resolution no longer match — Replit's
    // receiver rejects with "full_article length mismatch before insert"
    // (422, caught live 2026-07-08). Recompute both from the final string.
    if (typeof resolved.full_article === 'string') {
      resolved.full_article_length = resolved.full_article.length
      resolved.full_article_hash = sha256Hex(resolved.full_article)
    }
    return resolved
  }
...
export {
  ...
  normalizeClaims,
  truncateAtBoundary,
  // Peak Luxentria audit safety (re-exported from review-sync-safety)
  shapeRecentAdForSync,
  stripStructuredDupesFromArticle,
  sanitizeItemReviewedUrl,
  pickBrandOrigin,
