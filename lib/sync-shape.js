...
function normalizeDataset(raw, { brandName, brandSlug, siteUrl, observationWindow } = {}) {
  ...
  // fall back to the observation window from the SpyOwl monitoring dates
  ...
  const defaultKeywords = [brandName, 'scam intelligence', 'ad surveillance', 'SpyOwl'].filter(
    (v) => typeof v === 'string' && v.length > 0
  )

  return {
    '@id': siteUrl && brandSlug
      ? `${siteUrl}/review/${brandSlug}#spyowl-dataset`
      : null,
    name: raw.name,
...
function normalizeItemReviewed(raw, { brandName } = {}) {
  ...
  // No writer output → synthesize a minimal Service node from the brand
  // row so Replit has SOMETHING typed to work with. Service is the safest
  // fallback — every scam "provides" a fraudulent service of some kind.
  ...
  const type = VALID_TYPES.has(raw.type) ? raw.type : 'Service'
...
export function shapeReviewForSync(review, brand) {
  ...
  const adCreativeUrls = Array.isArray(review.ad_creative_urls)
    ? review.ad_creative_urls.filter((u) => typeof u === 'string' && u.startsWith('http'))
    : []
  ...
  // brand row's SpyOwl first/last seen timestamps (source of truth),
