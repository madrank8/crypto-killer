...
const VALID_PERSONAS = new Set(['webb', 'ortiz', 'krebs'])
...
function normalizeHowTo(raw) {
  ...
  for (const s of steps) {
    ...
    if (typeof s.name !== 'string' || typeof s.text !== 'string') continue
    cleanSteps.push({ name: s.name, text: s.text })
  }
...
function normalizeQuotes(raw) {
  ...
  for (const q of raw) {
    ...
    if (typeof q.text !== 'string' || typeof q.speakerName !== 'string') continue
    out.push({
      text: q.text,
      speakerName: q.speakerName,
      speakerSlug: typeof q.speakerSlug === 'string' ? q.speakerSlug : null,
      citationUrl: typeof q.citationUrl === 'string' ? q.citationUrl : null,
      publishedDate: typeof q.publishedDate === 'string' ? q.publishedDate : null,
    })
  }
  return out
}
