...
function normalizeItemReviewed(raw, { brandName } = {}) {
  ...
  // being valid schema.org types. We still accept them from the writer as
  // semantic input, but remap to the nearest Google-valid type on output:
  //   FinancialProduct → Product       (fake investment product)
  //   Service          → Organization  (the scam operation itself)
  const GOOGLE_TYPE_MAP = {
    ...
    // Already-valid passthrough — rows normalized after 2026-06-11 carry
    // these directly (e.g. the FinancialProduct→Product backfill).
    Product: 'Product',
  }
