...
function normalizeItemReviewed(raw, { brandName } = {}) {
  ...
  //
  // CONSTRAINT (verified live 2026-06-11): the Replit renderer has its OWN
  // type whitelist — {FinancialProduct, Service, SoftwareApplication,
  // Organization} with a 'Service' fallback. Sending 'Product' came back as
  // 'Service' on the live page. So output must stay inside the
  // INTERSECTION of Replit-accepted ∩ Google-valid types, which is exactly
  // {SoftwareApplication, Organization} — until the Replit-side whitelist
  // is extended (then FinancialProduct→Product becomes preferable).
  const GOOGLE_TYPE_MAP = {
    FinancialProduct: 'Organization',
