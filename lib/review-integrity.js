...
function checkReviewIntegrity(input = {}) {
  ...
  if (article) {
    for (const { field, token, label, detectRe: re } of BRAND_STATS) {
      let actual = brand[field]
      // Phase 1 review fix (2026-08-31): for celebrities the raw scraper tally
      // double-counts cross-script variants and undercounts compound entries
      // ("A, B" stored as one row). The LLM auditor's ground truth and the
      // canonical investigation model both use the deduped+split NAME count —
      // this deterministic check must agree with them, or the two publish
      // gates fight each other (immediate-bienestar: 61 real people, raw
      // tally 51; the article's 61 was flagged as fabricated).
      if (field === 'total_celebrities' && Array.isArray(brand.celebrity_list) && brand.celebrity_list.length > 0) {
        try {
          const { dedupeCelebrityList } = require('./threat-score')
          const named = dedupeCelebrityList(brand.celebrity_list).length
          const truncated = isNum(brand.total_celebrities) && brand.total_celebrities > brand.celebrity_list.length
          actual = truncated ? brand.total_celebrities : named
        } catch { /* fall back to the raw column */ }
      }
      re.lastIndex = 0
      let m
      const seen = new Set()
