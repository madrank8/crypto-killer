...
// PER-BRAND measured stats. Each has a {{stat:KEY}} token that resolves against
// the brand's live row, so any literal written in prose freezes at generation time
// and drifts as the scraper ingests more ads.
//
// Verified on real data rather than assumed — every one of these drifted:
//   quantum-ai  "45 countries"  -> total_geos 49
//   quantum-ai  "56 celebrities"-> total_celebrities 353
//   quantum-ai  "3,000+"        -> total_creatives 4,813
//   senvix      "1,107"         -> total_creatives 1,248
//   gaspipe-ai  "202"           -> total_creatives 204
// (kaspi-ai's 136 still matches — correct today, fragile tomorrow.)
const BRAND_STAT_PATTERNS = [
  {
    field: 'total_creatives', token: '{{stat:ad_creatives}}', label: 'ad creatives',
    re: /([\d][\d,\.]*)\s*\+?\s*(?:ad\s+)?creatives/gi,
  },
  {
    field: 'total_geos', token: '{{stat:countries_targeted}}', label: 'countries',
    re: /([\d][\d,\.]*)\s*\+?\s*(?:countries|geos|geographies|markets)/gi,
  },
  {
    field: 'total_celebrities', token: '{{stat:celebrities_abused}}', label: 'celebrities',
    re: /([\d][\d,\.]*)\s*\+?\s*(?:celebrities|celebrity\s+(?:impersonations?|abuses?))/gi,
  },
]

// PLATFORM-scale claims — these need {{platform_stat:KEY}}, not a per-brand token.
...
function checkReviewIntegrity(input = {}) {
  ...
  if (article) {
    for (const { field, token, label, re } of BRAND_STAT_PATTERNS) {
