...
/**
 * Extract Claim structures from {{VERIFY:...}} tags in section bodies.
 *
 * ── Audit 2026-07-05 (A2): these are Claim nodes, NOT ClaimReview. ──
 * A {{VERIFY: statement | source}} tag marks a statement the WRITER wants
 * confirmed — by definition unverified at resolve time. The previous
 * implementation converted every one into a ClaimReview with
 * ratingValue 5 / "Verified", i.e. a structured assertion that CryptoKiller
 * fact-checked the claim and confirmed it — which nobody ever did. On a
 * YMYL site that is fabricated structured data (the exact class the
 * comment below the loop already bans for the synthetic fallback).
 *
 * Each tag now becomes an honest Claim node:
 *   {
 *     "@type": "Claim",
 *     "text": <statement>,
 *     "author": { "@type": "Organization", "name": <source name> },   // when sourced
 *     "firstAppearance": <source url>,                                 // when sourced
 *   }
 *
 * Real ClaimReview nodes (ratingValue 1 "False" debunks of scam claims)
 * remain the review pipeline's job, where the debunk IS the fact-check.
 *
...
function extractClaims(article, context) {
  ...
  for (const section of sections) {
    ...
    for (const m of matches) {
      ...
      const claim = { '@type': 'Claim', text: parsed.statement }
      if (matched) {
        if (matched.publisher) {
          claim.author = { '@type': 'Organization', name: matched.publisher }
        }
        if (matched.url) claim.firstAppearance = matched.url
      } else if (parsed.source || parsed.url) {
        if (parsed.source) {
          claim.author = { '@type': 'Organization', name: parsed.source }
        }
        if (parsed.url) claim.firstAppearance = parsed.url
      }
      // baseUrl intentionally unused here — Claim nodes carry the SOURCE's
      // identity, not ours. We only claim authorship of a fact-check when
      // one actually happened (review-pipeline debunks).
      void baseUrl

      claims.push(claim)
    }
  }
  // NO synthetic-ClaimReview fallback. ClaimReview is a structured fact-check
  // assertion: emitting "Verified" (ratingValue 5) nodes that were never
  // actually fact-checked — synthesized from a bare source ledger when no
  // inline {{VERIFY}} tags survived — is fabricated structured data. On a YMYL
  // site that is an FTC 16 CFR 465 / Google structured-data manipulation risk
  // (a manual-action trigger), not a rich-result win. If the writer produced
  // no verifiable claims, the article simply ships without ClaimReview nodes;
  // rich-result coverage is earned by real {{VERIFY:claim|source}} tags, never
  // manufactured. (Audit: YMYL writing-process review.)
  return claims
}
...
function extractStepsFromBody(body) {
  ...
  if (h3Matches.length < 3) {
    // Fallback: detect ordered list steps — HTML <li> or markdown "1. item"
    // (audit 2026-07-05, A5: section bodies are MARKDOWN in the active
    // pipeline; the HTML-only fallback never matched them).
    const liMatches = [...body.matchAll(/<li[^>]*>([\s\S]*?)<\/li>/g)]
    let plainList = liMatches.map((m) => stripTags(m[1])).filter(Boolean)
    if (plainList.length < 3) {
      const mdItems = [...body.matchAll(/^\s*\d+[.)]\s+(.+)$/gm)]
        .map((m) => m[1].trim())
        .filter(Boolean)
      if (mdItems.length >= 3) plainList = mdItems
    }
    if (plainList.length >= 3) {
...
function extractQuotes(article) {
  ...
  for (const section of sections) {
    const body = String(section?.body || '')
    // HTML blockquotes + markdown "> quote — Attribution" lines (audit
    // 2026-07-05, A5: the active pipeline stores MARKDOWN section bodies;
    // the HTML-only regex meant quotes[] was always empty). Consecutive
    // "> " lines merge into one quote block, mirroring markdown semantics.
    const mdBlocks = []
    let current = []
    for (const line of body.split('\n')) {
      if (/^\s*>\s?/.test(line)) {
        ...
        mdBlocks.push(current.join(' '))
        current = []
      }
    }
    if (current.length) mdBlocks.push(current.join(' '))

    const matches = [
      ...[...body.matchAll(BLOCKQUOTE_RX)].map((m) => m[1]),
      ...mdBlocks,
    ]
