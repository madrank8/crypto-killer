...
async function runReviewPipeline(args, onProgress = () => {}) {
  ...
  // Normalize how_it_works to exactly 4 \n\n-separated paragraphs. The
  // Crest Fundgrove incident (2026-06-10): the writer emitted inline
  // "STAGE N —" markers in one paragraph, and downstream splitters
  // (full_article card builder, parseFunnelStages) produced corrupted
  // funnel cards. Enforce the contract here instead of trusting prompts.
  if (typeof core.how_it_works === 'string') {
    // Convert literal "\n" escape sequences (writer double-escaping) to real
    // newlines before splitting — same tolerance the article builder has.
    core.how_it_works = core.how_it_works.replace(/\\n/g, '\n')
    let hiwParas = core.how_it_works.split(/\n{2,}/).map((p) => p.trim()).filter(Boolean)
    if (hiwParas.length < 4) {
      const inlineSplit = core.how_it_works
        .split(/(?=\bSTAGE\s+[1-9]\s*(?:[—–\-:]|\())/i)
        .map((p) => p.trim())
        .filter((p) => p.length > 0)
      if (inlineSplit.length > hiwParas.length) hiwParas = inlineSplit
    }
    if (hiwParas.length > 4) {
      hiwParas = [...hiwParas.slice(0, 3), hiwParas.slice(3).join(' ')]
    }
    core.how_it_works = hiwParas.join('\n\n')
  }
