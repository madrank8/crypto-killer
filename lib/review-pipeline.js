...
The renderer substitutes {{stat:KEY}} tokens with live scraper values on every render. Emit tokens for these six stats wherever they appear in prose — NEVER the literal number. The values are deliberately WITHHELD: a number you write freezes into the page and drifts; a token stays live. You do not need the magnitude to write the sentence.
  ...
  Weekly velocity (7-day)   → {{stat:weekly_velocity}}
...
function buildCelebBlock(effectiveCelebList, derivedCelebCount, brandData) {
  return `═══ CELEBRITY REFERENCE HARD CONSTRAINT (YMYL — read twice) ═══
CELEBRITY NAMES — the COMPLETE, EXHAUSTIVE list of celebrities verified in ${brandData.name}'s ads (EXACT spellings only): ${effectiveCelebList.join(', ') || 'None detected'}

This list is the ONLY verified impersonation evidence for this brand. It is not a
sample or a starting point — it is the whole set. A name absent from it was NOT
observed in ${brandData.name}'s creatives, so naming that person as impersonated is
a fabricated, defamatory claim about a real individual on a YMYL page.

- Reference ONLY names from the list above — in prose, item_list, FAQ, examples, EVERYWHERE.
- ❌ Do NOT add a celebrity because you KNOW crypto scams often impersonate them
  (Elon Musk, Martin Lewis, etc.) — if they are not in the list, they are not evidence here.
- ❌ Do NOT add a locally-famous figure because it "fits" the targeted countries
  (e.g. an Indian businessperson for India traffic, an Australian politician for AU traffic).
  Plausibility is not evidence. This is exactly how fabricated names slip in.
- ❌ Do NOT infer, guess, complete, or "improve" the roster. Use it verbatim or fall back
  to generic phrasing ("public figures", "regional media personalities") with NO names.
- Empty list / "None detected" → NO specific names at all, only generic phrasing.
- CJK names: original script OR the exact romanization in the list — never an alternate.
- The celebrity COUNT in prose MUST be the token {{stat:celebrities_abused}}, never a
  literal (the list is WHO; the token is HOW MANY).

SELF-CHECK before returning: every proper name you wrote as an impersonated celebrity
appears CHARACTER-FOR-CHARACTER in the list above. If even one does not, remove it.
HARD REJECTION at audit: any celebrity name in the output not present in this list.`
}

function buildLedgerBlock(sourceLedger, brandData) {
  const lines = (sourceLedger || [])
    .map((s, i) => {
      const community = s.type === 'community_report' || s.class === 'community'
      const tag = s.generic ? ' [GENERIC]' : community ? ' [COMMUNITY]' : ''
      const dateSuffix = community && s.recency?.date ? ` (${s.recency.date})` : ''
      return `${i + 1}. [${s.type}]${tag} ${s.title}${dateSuffix} — ${s.url}${s.extract ? `\n   Extract: "${s.extract}"` : ''}`
    })
    .join('\n')
  const genericRule = (sourceLedger || []).some((s) => s.generic)
    ...
    : ''
  // last30days recency layer (gated on by RECENCY_EVIDENCE_ENABLED upstream).
  // Only emitted when community entries are actually present, so a normal run
  // is byte-identical to before.
  const communityRule = (sourceLedger || []).some((s) => s.type === 'community_report' || s.class === 'community')
    ? `\nCOMMUNITY-REPORT RULE ([COMMUNITY] entries — last-30-day posts, storm-research-grounded):
- Use as EXPERIENCE evidence, NEVER as proof of guilt. They corroborate the funnel and support experience_signals / red_flags framing ("victims reported in the last 30 days that…"). A regulator finding is still required to state any regulatory status.
- ATTRIBUTE + DATE every use: "A Reddit user reported on <date>…" — never an anonymous absolute claim.
- SCHEMA: a [COMMUNITY] URL may appear in quotes[] (citation = the post URL) and in citations[] typed CreativeWork or SocialMediaPosting ONLY. NEVER type it GovernmentService/Report, NEVER use it as a ClaimReview appearance, NEVER put it in a regulator sameAs.
- NUMBERS: only the engagement figure shown in a [COMMUNITY] extract is citable; otherwise stay qualitative ("multiple recent reports"). Do not invent counts.
- Do NOT let community reports alone upgrade ${brandData.name} from hedged to scam framing.`
    : ''
  return `═══ VERIFIED SOURCE LEDGER (URL-verified pre-write — use these, never invent URLs) ═══\n${lines || '(empty)'}${genericRule}${communityRule}`
}
...
function buildIntelligenceBlock(brandData, longevityDays, derivedCelebCount, creativeSample) {
  const samples = (creativeSample || []).slice(0, 8)
    ...
    .join('\n')
  return `INTELLIGENCE DATA (emit live stats as tokens, NOT literals — values withheld on purpose; you do not need them to write the sentence):
- Threat Score (LITERAL): ${brandData.scam_score}/100
- Ad Creatives: emit {{stat:ad_creatives}}
- Countries: emit {{stat:countries_targeted}}
- Celebrities: emit {{stat:celebrities_abused}}
- 7-Day Velocity: emit {{stat:weekly_velocity}}
- Velocity Trend (LITERAL): ${brandData.velocity_trend}
- Days Active: emit {{stat:days_active}}
- First Seen: emit {{stat:first_detected}}
- Last Seen: emit {{stat:last_active}}
...

{
  ...
  "experience_signals": ["3-5 first-person observations from investigating THIS brand that only an investigator would know. If you name a person, it MUST be from the CELEBRITY list above — do NOT map a celebrity to a country/geo (e.g. 'X fronts the UK ads, Y the Australian ones') unless every named person is in the list. When in doubt, describe the localization generically ('locally recognizable figures', 'region-appropriate public faces') with NO names."],
...
5. The remaining 1-4 items MUST be brand-specific (a celebrity used in the ads, the dominant target country, the funnel mechanic) — NEVER reuse a generic question that would fit every review. Identical FAQ sets across thousands of reviews are a scaled-content fingerprint (R43). If a question or answer names a celebrity, it MUST come from the CELEBRITY list above — never pair a named person with a country ("Why does X use <Name> in its <Country> ads?") unless that person is in the list; use a list name or generic phrasing instead.`
