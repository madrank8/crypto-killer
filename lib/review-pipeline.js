...
${String(core.how_it_works || '').slice(0, 2500)}`
    try {
      const retry = await runStage('core-retry', ['claude-opus', 'claude-sonnet'], co.system, correctiveUser, { maxTokens: 4096, timeoutMs: 240000 }, attempts)
      const retryNorm = normalizeHowItWorks(retry.json.how_it_works)
      if (retryNorm.count >= hiwNorm.count) {
        core = { ...retry.json }
        hiwNorm = retryNorm
      }
    } catch (retryErr) {
