...
import { materializeFromReport } from './work-plan'
...
export async function runAdvisor({ trigger = 'manual', days = 28 } = {}) {
  ...
  // Best-effort: push suggestions into the Work Plan queue.
  try {
    await materializeFromReport(stored)
  } catch (err) {
    console.warn('[advisor] work-plan materialize failed:', err.message)
  }
