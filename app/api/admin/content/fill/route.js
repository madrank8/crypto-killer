    ...
    const stream = new ReadableStream({
      async start(controller) {
        ...
        try {
          ...
          // One-shot Quality Fix Agent when hard fails remain (no recurse).
          let qualityFixResult = null
          if (qualityFixAutoEnabled()) {
            const mergedRow = { ...content, ...patch }
            const hardFails = evaluateHardFails(patch.ai_audit, mergedRow).failed
            if (hardFails.length > 0) {
              send({
                step: 'quality_fix',
                progress: 92,
                message: `Running quality fix agent (${hardFails.length} hard fail${hardFails.length !== 1 ? 's' : ''})…`,
              })
              try {
                ...
                console.error('[fill] Quality fix failed:', fixErr.message)
                send({
                  step: 'quality_fix',
                  progress: 98,
          ...
          send({
            ...
            progress: 100,
            message: qualityFixResult?.published
              ? `Article generated by ${personaMetadata.name} — quality fix published.`
              : `Article generated successfully by ${personaMetadata.name} — review and publish when ready.`,
            result: {
